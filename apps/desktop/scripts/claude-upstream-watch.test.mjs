...
test("watcher reads the dependency version, not the private package version", async () => {
    ...
    assert.equal(readPinnedClaudeVersion({ ...manifest, version: "9.0.0" }), "0.76.0");
    assert.equal(compareSemver(parseSemver("v0.76.1"), parseSemver(readPinnedClaudeVersion(manifest))), 1);
});

test("watcher rejects floating, preview and missing runtime pins", () => {
    for (const version of [undefined, "latest", "^0.76.0", "~0.76.0", "0.76.0-preview.1", "v0.76.0"]) {
