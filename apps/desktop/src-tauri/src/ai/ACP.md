...
Current Claude vendor notes (`0.29.0`):
...
- Claude now emits `usage_update` notifications from stream events, not only from the terminal `result`
- Claude now carries forward a better context-window guess for `1m` models until the authoritative `modelUsage` arrives
...
│
vendor/
├── Claude-agent-acp-upstream/        # @agentclientprotocol/claude-agent-acp v0.29.0
│   ├── dist/index.js
│   ├── node_modules/
│   └── package.json
