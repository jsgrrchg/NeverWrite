...
use agent_client_protocol::{
    ...
    InitializeResponse, ListSessionsRequest, LoadSessionRequest, Meta, NewSessionRequest,
    PermissionOption, PromptRequest, ProtocolVersion, RequestPermissionOutcome,
    RequestPermissionRequest, RequestPermissionResponse, Result as AcpResult, ResumeSessionRequest,
...
use neverwrite_ai::{
    ...
    AiRuntimeSessionSummary, AiSession, KILO_RUNTIME_ID,
};
...
use super::{process::KiloProcessSpec, setup::apply_auth_env};
...
enum RuntimeCommand {
    CreateSession {
        spec: KiloProcessSpec,
        additional_roots: Option<Vec<String>>,
        response_tx: mpsc::Sender<Result<KiloSessionState, String>>,
    },
    LoadSession {
        spec: KiloProcessSpec,
        session_id: String,
...
struct ToolState {
    ...
    /// Pre-write file baselines captured when Kilo reads a file.
...
impl ToolState {
    ...
    /// Cache the original file content when Kilo reads a file.
    ...
    fn drop(&mut self) {
        ...
        if !self.queue_drained {
            let _ = self.command_tx.send(RuntimeCommand::PromptFinished {
                ...
                result: Err("Kilo prompt task ended unexpectedly.".to_string()),
            });
        }
    }
}
...
struct NeverWriteAcpClient {
    ...
    session_cache: KiloSessionCache,
}

#[derive(Debug, Clone)]
pub struct KiloSessionState {
...
#[derive(Debug, Clone, Default)]
pub(crate) struct KiloSessionCache {
    sessions: Arc<Mutex<HashMap<String, AiSession>>>,
}

impl KiloSessionCache {
...
pub struct KiloNegotiatedCapabilities {
...
impl KiloNegotiatedCapabilities {
    fn from_initialize_response(response: &InitializeResponse) -> Self {
        Self {
            ...
            set_config_option: response
                ...
                .meta
                .as_ref()
                .and_then(|meta| meta.get("supportsSetSessionConfigOption"))
                .and_then(serde_json::Value::as_bool)
                .unwrap_or(false),
            terminal_output: true,
            prompt_queueing: false,
        }
    }
}
...
#[derive(Debug, Clone)]
pub struct KiloRuntimeHandle {
    command_tx: tokio_mpsc::UnboundedSender<RuntimeCommand>,
    negotiated_capabilities: Arc<Mutex<KiloNegotiatedCapabilities>>,
}

impl KiloRuntimeHandle {
    pub fn spawn(app: AppHandle, session_cache: KiloSessionCache) -> Self {
        let (command_tx, command_rx) = tokio_mpsc::unbounded_channel::<RuntimeCommand>();
        let actor_command_tx = command_tx.clone();
        let negotiated_capabilities = Arc::new(Mutex::new(KiloNegotiatedCapabilities::default()));
        let actor_negotiated_capabilities = negotiated_capabilities.clone();
    ...
    pub fn create_session(
        ...
        spec: KiloProcessSpec,
        additional_roots: Option<Vec<String>>,
    ) -> Result<KiloSessionState, String> {
    ...
    pub fn negotiated_capabilities(&self) -> KiloNegotiatedCapabilities {
...
async fn run_actor(
    ...
    let mut actor = RuntimeActor::new(app, command_tx, session_cache, negotiated_capabilities);
...
impl RuntimeActor {
    ...
    fn reset_connection(&mut self) {
        ...
        if let Ok(mut guard) = self.negotiated_capabilities.lock() {
            *guard = KiloNegotiatedCapabilities::default();
        }
    ...
    async fn ensure_connection(
        ...
        let stdin = child
            .stdin
            .take()
            .ok_or_else(|| "Failed to acquire kilo stdin".to_string())?;
        let stdout = child
            .stdout
            .take()
            .ok_or_else(|| "Failed to acquire kilo stdout".to_string())?;
        let stderr = child
            .stderr
            .take()
            .ok_or_else(|| "Failed to acquire kilo stderr".to_string())?;
    ...
    async fn ensure_initialized(
        ...
        let request = InitializeRequest::new(ProtocolVersion::LATEST)
            .client_capabilities(
                ClientCapabilities::new()
                    ...
                    .terminal(false)
                    .meta(Meta::from_iter([
                        ("terminal-auth".to_string(), serde_json::json!(true)),
                        ("terminal_output".to_string(), serde_json::json!(true)),
                    ])),
            )
            .client_info(
        ...
        let response = connection
            ...
            .map_err(|error| error.to_string())?;
        if let Ok(mut guard) = self.negotiated_capabilities.lock() {
            *guard = KiloNegotiatedCapabilities::from_initialize_response(&response);
        }
    ...
    async fn create_session(
        ...
        let cwd = spec
            .cwd
            .clone()
            .ok_or_else(|| "No hay vault abierto para iniciar una sesion Kilo.".to_string())?;
    ...
    async fn load_session(
        ...
        let cwd = spec
            .cwd
            .clone()
            .ok_or_else(|| "No hay vault abierto para cargar una sesion Kilo.".to_string())?;

        let connection = self.ensure_initialized(&spec).await?;
        if !self
            .negotiated_capabilities
            .lock()
            .map(|guard| guard.load_session)
            .unwrap_or(false)
        {
            return Err("Kilo ACP runtime does not advertise session/load support.".to_string());
        }
        let response = connection
    ...
    async fn list_sessions(
        ...
        if !self
            .negotiated_capabilities
            .lock()
            .map(|guard| guard.list_sessions)
            .unwrap_or(false)
        {
            return Err("Kilo ACP runtime does not advertise session/list support.".to_string());
        }
        let response = connection
    ...
    async fn resume_session(
        ...
        let cwd = spec
            .cwd
            .clone()
            .ok_or_else(|| "No hay vault abierto para reanudar una sesion Kilo.".to_string())?;

        let connection = self.ensure_initialized(&spec).await?;
        if !self
            .negotiated_capabilities
            .lock()
            .map(|guard| guard.resume_session)
            .unwrap_or(false)
        {
            return Err("Kilo ACP runtime does not advertise session/resume support.".to_string());
        }
        let response = connection
    ...
    async fn fork_session(
        ...
        let cwd = spec
            .cwd
            .clone()
            .ok_or_else(|| "No hay vault abierto para bifurcar una sesion Kilo.".to_string())?;

        let connection = self.ensure_initialized(&spec).await?;
        if !self
            .negotiated_capabilities
            .lock()
            .map(|guard| guard.fork_session)
            .unwrap_or(false)
        {
            return Err("Kilo ACP runtime does not advertise session/fork support.".to_string());
        }
        let response = connection
    ...
    async fn set_config_option(
        ...
        if !self
            .negotiated_capabilities
            .lock()
            .map(|guard| guard.set_config_option)
            .unwrap_or(false)
        {
            return Err(
                "Kilo ACP runtime does not advertise session/config option updates.".to_string(),
            );
        }
        self.poll_connection_health()?;
        let connection = self
...
fn reconstruct_edit_diff_payload(
    ...
    // Read the current file from disk (post-edit, already written by Kilo)
...
mod tests {
    ...
    fn test_session() -> AiSession {
        AiSession {
            ...
            runtime_id: KILO_RUNTIME_ID.to_string(),
            model_id: "kilo-3-5-sonnet".to_string(),
            ...
            config_options: vec![
                test_config_option(
                    ...
                    "kilo-3-5-sonnet",
                    vec![
                        test_select_option("kilo-3-5-sonnet"),
                        test_select_option("kilo-3-7-sonnet"),
                    ],
                ),
                test_config_option(
                    "mode",
    ...
    fn apply_config_options_updates_config_options_and_derived_ids() {
        ...
        assert_eq!(session.model_id, "kilo-3-7-sonnet");
        assert_eq!(session.mode_id, "plan");
    ...
    fn unique_temp_dir() -> PathBuf {
        let suffix = SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        std::env::temp_dir().join(format!("neverwrite-kilo-client-{suffix}"))
    }
    ...
    fn reconstruct_edit_diff_from_raw_input() {
        ...
        // File already contains the post-edit content (Kilo wrote it)
    ...
    fn write_diff_caches_update_marker_when_file_already_written() {
        ...
        // File already contains the content Kilo wants to write (post-write state)
    ...
    fn second_upsert_does_not_overwrite_first_cached_diff() {
        ...
        // Simulate Kilo writing the file
        fs::write(&file_path, "new content").unwrap();
    ...
    fn read_tool_caches_baseline_for_subsequent_write() {
        ...
        // Simulate Kilo reading the file (completed Read tool)
        ...
        // Now Kilo writes the file (auto-approved, file already written)
        fs::write(&file_path, "new content").unwrap();
    ...
    fn store_file_baseline_from_external_source() {
        ...
        // Kilo writes the file (already written to disk)
        fs::write(&file_path, "kilo content").unwrap();
        let write_call = ToolCall::new(ToolCallId::from("tool-write-ext"), "Write external.md")
            .kind(ToolKind::Edit)
            .status(ToolCallStatus::Completed)
            .raw_input(serde_json::json!({
                "file_path": file_path.display().to_string(),
                "content": "kilo content",
            }));
        let registered = tool_state.upsert_tool_call("session-1", write_call);
        ...
        assert_eq!(diffs[0].new_text.as_deref(), Some("kilo content"));
        assert!(diffs[0].reversible);
