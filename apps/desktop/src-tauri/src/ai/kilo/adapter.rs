...
use super::{
    ...
    process::KiloRuntime,
    setup::{clear_authenticated_method, save_setup_config},
    KiloSetupInput,
};

#[derive(Debug, Default)]
pub struct KiloRuntimeAdapter {
    runtime: KiloRuntime,
    handle: Option<KiloRuntimeHandle>,
    session_cache: KiloSessionCache,
}

impl AiRuntimeAdapter for KiloRuntimeAdapter {
    ...
    fn capabilities(&self) -> AiRuntimeCapabilities {
        if let Some(handle) = self.handle.as_ref() {
            let negotiated = handle.negotiated_capabilities();
            return AiRuntimeCapabilities {
                create_session: true,
                fork_session: negotiated.fork_session,
                resume_session: negotiated.resume_session,
                list_sessions: negotiated.list_sessions,
                prompt_queueing: negotiated.prompt_queueing,
                terminal_output: negotiated.terminal_output,
    ...
    fn update_setup(
        ...
        let input: KiloSetupInput = input.into_kilo()?;
    ...
    fn start_auth(
        ...
        match method_id {
            "kilo-login" => Err(
                "Kilo authentication must be launched through the integrated sign-in terminal."
                    .to_string(),
            ),
            other => Err(format!("Unsupported Kilo auth method: {other}")),
        }
    }
    ...
    fn create_session(
        ...
        {
            return Err(
                "Kilo ACP does not support additional roots in this NeverWrite build yet."
                    .to_string(),
            );
        }

        let process_spec = self.runtime.process_spec(app, vault_root)?;
        let created = match self.handle(app).create_session(process_spec, None) {
            Ok(session) => session,
            Err(error) => {
    ...
    fn set_model(&mut self, session_id: &str, model_id: &str) -> Result<AiSession, String> {
        let supports_model = self
            ...
            .unwrap_or(false);
        if !supports_model {
            return Err(format!("Modelo no soportado por Kilo ACP: {model_id}"));
        }
    ...
    fn set_mode(&mut self, session_id: &str, mode_id: &str) -> Result<AiSession, String> {
        let supports_mode = self
            ...
            .unwrap_or(false);
        if !supports_mode {
            return Err(format!("Modo no soportado por Kilo ACP: {mode_id}"));
        }
    ...
    fn set_config_option(
        ...
        if !supports_value {
            return Err(format!(
                "Opcion invalida para Kilo ACP: {option_id}={value}"
            ));
        }
    ...
    fn respond_user_input(
        ...
        Err("Kilo does not support user input requests in this build.".to_string())
    }
...
impl KiloRuntimeAdapter {
    fn handle(&mut self, app: &AppHandle) -> KiloRuntimeHandle {
        if let Some(handle) = self.handle.as_ref() {
            return handle.clone();
        }

        let handle = KiloRuntimeHandle::spawn(app.clone(), self.session_cache.clone());
        self.handle = Some(handle.clone());
        handle
    }

    fn handle_from_session(&self, session_id: &str) -> Result<KiloRuntimeHandle, String> {
...
fn is_authentication_error(message: &str) -> bool {
    ...
    normalized.contains("auth_required")
        ...
        || normalized.contains("run `kilo auth login`")
        || normalized.contains("you were signed out")
        || normalized.contains("reconnect kilo")
}
