...
function deriveSpanLineRange(
    ...
    if (patch.edits.length > 0) {
        return {
            oldStartLine: Math.min(...patch.edits.map((edit) => edit.oldStart)),
            oldEndLine: Math.max(...patch.edits.map((edit) => edit.oldEnd)),
            newStartLine: Math.min(...patch.edits.map((edit) => edit.newStart)),
            newEndLine: Math.max(...patch.edits.map((edit) => edit.newEnd)),
        };
    }
