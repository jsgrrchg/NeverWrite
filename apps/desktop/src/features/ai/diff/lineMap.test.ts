...
describe("lineMap", () => {
  ...
  it("keeps EOF point ranges anchored to the last real line", () => {
    const text = "alpha\nbeta\ngamma";
    const lineStarts = buildLineStartOffsets(text);

    expect(insertionLineIndexAtOffset(lineStarts, text.length)).toBe(2);
    expect(deriveOffsetLineRange(lineStarts, text.length, text.length)).toEqual(
      {
        start: 2,
        end: 2,
      },
