...
function saveAiPreferences(patch: Partial<AiPreferences>) {
  try {
    ...
    safeStorageSetItem(AI_PREFS_KEY, JSON.stringify({ ...current, ...patch }));
  } catch {
...
function getPersistedHistoryCatalogSnapshot(
  history: Pick<PersistedSessionHistory, "models" | "modes" | "config_options">,
): AIRuntimeCatalogSnapshot {
  return {
...
async function ensureSessionAgentCatalogLoaded(
  ...
  if (!sessionHasAgentCatalog(session)) {
    ...
    if (persisted) {
      useChatStore
        .getState()
        .upsertSession(applyPersistedHistoryMetadata(session, persisted));
      session =
    ...
    if (session.runtimeState !== "live") {
      ...
      session = useChatStore.getState().sessionsById[liveSessionId] ?? null;
      if (!session) {
        return null;
      }
    }
    ...
    if (!sessionHasAgentCatalog(session)) {
      try {
        ...
        useChatStore.getState().upsertSession({
          ...loaded,
          historySessionId: latest.historySessionId ?? loaded.historySessionId,
          vaultPath: latest.vaultPath ?? loaded.vaultPath ?? null,
          persistedCreatedAt:
            latest.persistedCreatedAt ?? loaded.persistedCreatedAt ?? null,
          persistedUpdatedAt:
            latest.persistedUpdatedAt ?? loaded.persistedUpdatedAt ?? null,
          persistedTitle:
            latest.persistedTitle ?? loaded.persistedTitle ?? null,
          customTitle: latest.customTitle ?? loaded.customTitle ?? null,
          persistedPreview:
            latest.persistedPreview ?? loaded.persistedPreview ?? null,
...
function getConfigOptionValue(
  ...
  return session.configOptions.find((option) => option.id === optionId)?.value;
}
...
function applyLocalModelSelection(
  ...
  return {
    ...
    configOptions: session.configOptions.map((option) =>
      option.category === "model" ? { ...option, value: modelId } : option,
    ),
  };
}
...
function sessionReflectsAgentSelectionChange(
  ...
  switch (change.kind) {
    ...
    case "config":
      ...
      return getConfigOptionValue(session, change.optionId) === change.value;
  }
}
...
interface ChatStore {
  ...
  attachFolder: (folderPath: string, name: string, sessionId?: string) => void;
...
function createPlanMessage(payload: AIPlanUpdatePayload): AIChatMessage {
  ...
  const stepsContent = payload.entries.map((entry) => entry.content).join("\n");
...
function collectExternalAdditionalRoots(
  ...
  for (const attachment of attachments) {
    const candidateRoot = getAdditionalRootCandidateForAttachment(attachment);
    if (!candidateRoot) {
      continue;
    }
...
function snapshotSessionLocalState(
  ...
  return {
    composerParts:
      state.composerPartsBySessionId[sessionId] ?? createEmptyComposerParts(),
...
function migrateSessionLocalState(
  ...
  useChatStore.setState((state) => {
    ...
    if (localState.pausedQueue) {
      nextPausedQueueBySessionId[toSession.sessionId] = localState.pausedQueue;
    }
...
function registerOpenEditorBaselines(sessionId: string) {
  ...
  for (const tab of tabs) {
    if (isNoteTab(tab) && tab.content != null) {
      aiRegisterFileBaseline(sessionId, `${tab.noteId}.md`, tab.content).catch(
        () => {},
      );
      continue;
    }
...
function finalizeQueuedMessageEditState(
  ...
  const nextQueuedMessagesBySessionId = cleanupQueuedMessagesBySessionId(
    ...
    sessionId,
    restoredItem
      ? restoreQueuedMessagePosition(sanitizedQueue, editState, restoredItem)
      : sanitizedQueue,
  );
...
async function replaceEmptySessionForAdditionalRoots(
  ...
  if (
    !latestSession ||
    !canRecreateSessionForAdditionalRoots(latestSession, sessionId, latestState)
  ) {
...
function reconcileIdleQueuedState(
  ...
  const activeQueuedMessageIsSending =
    ...
    (activeQueuedMessage != null &&
      queue.some(
        (item) =>
          item.id === activeQueuedMessage.item.id && item.status === "sending",
      ));
...
async function hasConflict(
  ...
  if (tracked.originPath !== tracked.path) {
    const originHash = await aiGetTextFileHash(vaultPath, tracked.originPath);
    if (originHash !== null) {
...
function findTrackedFileForIncomingDiff(
  ...
  return (
    Object.values(files).find((file) =>
      matchesTrackedFileForDebug(file, diff),
    ) ?? null
  );
}
...
function getDiffNormalizationCandidates(
  ...
  const tracked = findTrackedFileForIncomingDiff(currentFiles, diff);
  ...
  for (const path of targetPaths) {
    pushCandidate(resolveEditorTargetForTrackedPath(path)?.openTab?.content);
  }
...
function normalizeIncomingTrackedDiff(
  ...
  const nextFragment = diff.kind === "delete" ? "" : (diff.new_text ?? "");
  for (const candidate of getDiffNormalizationCandidates(diff, currentFiles)) {
    if (candidate === diff.old_text || candidate === nextFragment) {
      continue;
    }
    ...
    logDebug(
      "tracked-review",
      "expanded fragmentary diff from editor baseline",
      {
        path: diff.path,
        previousPath: diff.previous_path ?? null,
        kind: diff.kind,
        oldTextLength: diff.old_text.length,
        newTextLength: nextFragment.length,
    ...
    return {
      ...
      old_text: candidate,
      kind:
        diff.kind === "delete" && reconstructed.length > 0
          ? "update"
          : diff.kind,
      new_text:
        diff.kind === "delete" && reconstructed.length > 0
...
function normalizeIncomingTrackedDiffs(
  ...
  return diffs.map((diff) => normalizeIncomingTrackedDiff(diff, currentFiles));
}
...
function consolidateActionLogDiffs(
  ...
  const normalizedDiffs = normalizeIncomingTrackedDiffs(currentFiles, diffs);
  ...
  if (relevantBefore.length > 0 || relevantAfter.length > 0) {
    logDebug("tracked-review", "consolidate agent diffs", {
      ...
      workCycleId,
      diffs: normalizedDiffs.map((diff) => ({
        path: diff.path,
        previousPath: diff.previous_path ?? null,
        kind: diff.kind,
        oldTextLength: diff.kind === "add" ? 0 : (diff.old_text ?? "").length,
        newTextLength:
...
function removeTrackedFileFromActionLog(
  ...
  const matchingKey =
    Object.keys(files).find(
      (key) => key === identityKey || files[key]?.identityKey === identityKey,
    ) ?? null;
...
async function executeRestoreAction(
  ...
  if (action.kind === "delete") {
    ...
    change = await aiRestoreTextFile({
      vaultPath,
      path:
        tracked.originPath !== tracked.path ? tracked.originPath : tracked.path,
      previousPath:
        action.previousPath ??
...
function applyUserEditToTrackedFileInSession(
  ...
  useChatStore.setState((state) => {
    ...
    const updated = applyNonConflictingEdits(tracked, userEdits, newFullText);
...
function getSessionPersistedWindowStart(session: AIChatSession) {
  ...
  if ((session.persistedMessageCount ?? 0) > 0 && session.messages.length > 0) {
...
function hasFullPersistedTranscriptLoaded(session: AIChatSession) {
  ...
  return (
    session.loadedPersistedMessageStart === 0 || session.runtimeState !== "live"
  );
}
...
function withUniqueAttachment(
  ...
  if (next.noteId) {
    const duplicate = attachments.some(
      (attachment) =>
        attachment.type === next.type && attachment.noteId === next.noteId,
    );
...
function mergeSession(
  ...
  const merged = {
    ...
    ...incoming,
    models:
      incoming.models.length > 0 ? incoming.models : normalizedExisting.models,
    modes:
      incoming.modes.length > 0 ? incoming.modes : normalizedExisting.modes,
    configOptions:
    ...
    isPersistedSession:
      incoming.isPersistedSession ?? normalizedExisting.isPersistedSession,
    resumeContextPending:
      incoming.resumeContextPending ?? normalizedExisting.resumeContextPending,
    activeWorkCycleId:
    ...
    visibleWorkCycleId:
      ...
      null,
    effortsByModel:
      incoming.effortsByModel && Object.keys(incoming.effortsByModel).length > 0
        ? incoming.effortsByModel
        : (normalizedExisting.effortsByModel ?? incoming.effortsByModel ?? {}),
    availableCommands:
      incoming.availableCommands && incoming.availableCommands.length > 0
        ? incoming.availableCommands
        : (normalizedExisting.availableCommands ?? incoming.availableCommands),
    persistedCreatedAt:
    ...
    persistedUpdatedAt:
      ...
      null,
    persistedTitle:
      incoming.persistedTitle ?? normalizedExisting.persistedTitle ?? null,
    customTitle: incoming.customTitle ?? normalizedExisting.customTitle ?? null,
    persistedPreview:
      incoming.persistedPreview ?? normalizedExisting.persistedPreview ?? null,
    persistedMessageCount:
  ...
  useChatStore.setState((state) => {
    ...
    for (const [sessionId, { message_id, text }] of msgEntries) {
      ...
      if (
        ...
        const idTaken = normalizedSession.messagesById?.[message_id] != null;
...
export const useChatStore = create<ChatStore>((set, get) => {
  async function loadPersistedTranscript(
    ...
    if (persistedCount === 0) {
      ...
      set((state) => {
        ...
        return {
          sessionsById: updateSessionById(state, sessionId, (nextSession) => {
            if (
    ...
    const currentStart = session.loadedPersistedMessageStart ?? persistedCount;
    const startIndex =
      mode === "full"
        ? 0
        : mode === "older"
    ...
    try {
      ...
      if (!isPersistedHistoryPage(payload)) {
        throw new Error("Persisted transcript page payload is invalid.");
      }
      const page = payload;
  ...
  function patchQueuedMessage(
    ...
    set((state) => {
      ...
      return nextQueuedMessagesBySessionId === state.queuedMessagesBySessionId
        ? state
        : {
            queuedMessagesBySessionId: nextQueuedMessagesBySessionId,
          };
  ...
  function updateActiveQueuedMessage(
    sessionId: string,
    updater: (deferredMessage: DeferredQueuedMessage) => DeferredQueuedMessage,
  ) {
    set((state) => {
      const deferredMessage = state.activeQueuedMessageBySessionId[sessionId];
      if (!deferredMessage) {
        return state;
      }
  ...
  function restoreActiveQueuedMessage(
    ...
    set((state) => {
      ...
      const currentQueue = state.queuedMessagesBySessionId[sessionId] ?? [];
  ...
  async function ensureRuntimeVisibleAfterOnboarding(runtimeId: string) {
    ...
    if (
      runtimeSupportsCapability(state.runtimes, runtimeId, "create_session")
    ) {
  ...
  async function dispatchMessage(
    ...
    try {
      ...
      if (
        ...
        updateActiveQueuedMessage(activeSessionId, (deferredMessage) => ({
          ...deferredMessage,
          item: {
            ...deferredMessage.item,
            ...
            [activeSessionId]: upsertSessionMessage(
              {
                ...
                status: "streaming",
                attachments:
                  source === "immediate" && !options?.preserveComposerState
                    ? []
  ...
  async function flushPendingInterruptedSend(
    ...
    if (get().pausedQueueBySessionId[sessionId]) {
      ...
      const nextQueuedMessageId = (get().queuedMessagesBySessionId[sessionId] ??
        [])[0]?.id;
      if (
        nextQueuedMessageId &&
  ...
  return {
    ...
    initialize: async (options) => {
      ...
      const shouldCreateDefaultSession = options?.createDefaultSession ?? true;

      set({ isInitializing: true });

      try {
        const runtimes = hydrateRuntimesFromCache(await aiListRuntimes());
        const runtimeIds = runtimes.map((descriptor) => descriptor.runtime.id);
        ...
        const setupStatusByRuntimeId = buildSetupStatusMap(setupStatuses);
        ...
        if (sessions.length || histories.length) {
          set((state) => {
            ...
            >((accumulator, session) => {
              const scopedSession = stampSessionVaultPath(session, vaultPath);
              ...
              if (persisted) {
                merged = applyPersistedHistoryMetadata(merged, persisted);
              }
                ...
                merged = replaceSessionTranscript(
                  {
                    ...
                    loadedPersistedMessageStart: Math.max(
                      0,
                      getPersistedHistoryMessageCount(persisted) -
            ...
            const nextSessionOrder = sortSessionIdsByRecency(nextSessionsById);
            const nextActiveSessionId =
              state.activeSessionId && nextSessionsById[state.activeSessionId]
                ? state.activeSessionId
          ...
          if (
            ...
            get().sessionsById[nextActiveSessionId]!.runtimeState !== "live"
          ) {
          ...
          if (hydratedActiveSessionId) {
            await ensureSessionAgentCatalogLoaded(hydratedActiveSessionId);
          }
          return;
        }
        ...
        const runtimeId =
          get().selectedRuntimeId ?? getDefaultRuntimeId(get().runtimes);
        if (runtimeId) {
          set((state) => ({
            runtimeConnectionByRuntimeId: setRuntimeConnectionState(
              state.runtimeConnectionByRuntimeId,
    ...
    reconcileRestoredWorkspaceTabs: async (tabs, activeTabId = null) => {
      ...
      set((state) => {
        ...
        for (const tab of tabs) {
          ...
          if (
            currentSession.historySessionId !== tab.historySessionId ||
          ...
          if (persisted) {
            nextSession = applyPersistedHistoryMetadata(nextSession, persisted);
          }
      ...
      if (
        ...
        const resumedSessionId = await get().resumeSession(activeSessionId);
        activeSession =
          (resumedSessionId ? get().sessionsById[resumedSessionId] : null) ??
          null;
      }
    ...
    saveSetup: async (input) => {
      const targetRuntimeId = input.runtimeId ?? getEffectiveRuntimeId(get());
      ...
      try {
        ...
        if (!setupStatus.onboardingRequired) {
          ...
          if (previousSetupStatus?.onboardingRequired) {
            await ensureRuntimeVisibleAfterOnboarding(setupStatus.runtimeId);
          } else if (
    ...
    upsertSession: (session, activate = false) => {
      ...
      set((state) => {
        ...
        const stampedSession = stampSessionVaultPath(session, currentVaultPath);
        const scopedSession = hydrateSessionCatalogFromRuntime(
          stampedSession,
          state.runtimes.find(
            (runtime) => runtime.runtime.id === stampedSession.runtimeId,
          ),
        );
        const existing = state.sessionsById[scopedSession.sessionId];
        const isKnown = state.sessionOrder.includes(scopedSession.sessionId);
        ...
        if (
          ...
          ((existing &&
            ...
            !isWorkspaceSession) ||
            !sessionMatchesVaultPath(scopedSession, currentVaultPath))
        ) {
          return state;
        }
        ...
        const nextRuntimes = scopedSession.isPersistedSession
          ? state.runtimes
          : hydrateRuntimesFromSessions(state.runtimes, [scopedSession]);
        ...
        const reconciledQueueState =
          ...
          !nextSession.isResumingSession
            ? reconcileIdleQueuedState(state, scopedSession.sessionId)
            : null;
        shouldDrainQueue =
          nextSession.status === "idle" &&
          (existing?.status !== "idle" || Boolean(reconciledQueueState)) &&
          !nextSession.isResumingSession;
        ...
        return {
          ...
          sessionOrder: activate
            ? touchSessionOrder(state.sessionOrder, scopedSession.sessionId)
            : state.sessionOrder,
          activeSessionId:
          ...
          composerPartsBySessionId: state.composerPartsBySessionId[
            ...
            : {
                ...state.composerPartsBySessionId,
                [scopedSession.sessionId]: createEmptyComposerParts(),
              },
          ...
          const revertedSession = finalizeActionLogForWorkCycle(
            stampElapsedOnTurnStartedSession(
              appendSessionError(
                ...
                message ?? "The AI runtime disconnected unexpectedly.",
              ),
              failedAt,
            ),
          );
        ...
        return changed
          ? {
              ...
              ...affectedSessionIds.reduce(
                  ...
                  return {
                    queuedMessagesBySessionId: cleanupQueuedMessagesBySessionId(
                      ...
                      restoreDeferredQueuedMessage(
                        nextState.queuedMessagesBySessionId[sessionId] ?? [],
                        {
                          ...activeQueuedMessage,
                          item: {
                ...
                {
                  queuedMessagesBySessionId: state.queuedMessagesBySessionId,
                  activeQueuedMessageBySessionId:
                    state.activeQueuedMessageBySessionId,
                  interruptedTurnStateBySessionId:
                    state.interruptedTurnStateBySessionId,
      ...
      set((state) => {
        ...
        if (!session_id || !state.sessionsById[session_id]) {
          return {
            ...
            runtimeConnectionByRuntimeId: nextRuntimeConnectionByRuntimeId,
          };
        }
              ...
              appendSessionError(
                {
                  ...revertedSession,
                  status: "error",
                  runtimeState: revertedSession.runtimeState ?? "live",
                },
                message,
              ),
              failedAt,
        ...
        return {
          ...
          queuedMessagesBySessionId: activeQueuedMessage
            ? cleanupQueuedMessagesBySessionId(
                state.queuedMessagesBySessionId,
                session_id,
                (state.queuedMessagesBySessionId[session_id] ?? []).filter(
                  (item) => item.id !== activeQueuedMessage.item.id,
                ),
              )
            : state.queuedMessagesBySessionId,
          activeQueuedMessageBySessionId:
    ...
    applyThinkingStarted: ({ session_id, message_id }) => {
      ...
      set((state) => {
        ...
        const exists = nextSession.messageIndexById?.[message_id] != null;
    ...
    applyThinkingCompleted: ({ session_id, message_id }) => {
      ...
      set((state) => {
        ...
        return {
          sessionsById: {
            ...state.sessionsById,
            [session_id]: setMessageInProgressState(session, message_id, false),
          },
          sessionOrder: touchSessionOrder(state.sessionOrder, session_id),
        };
      });
    ...
    applyToolActivity: (payload) => {
      ...
      set((state) => {
        ...
        const nextSession = toolActivityKeepsSessionStreaming(payload.status)
    ...
    applyPermissionRequest: (payload) => {
      ...
      set((state) => {
        ...
        workCycleId = nextSession.activeWorkCycleId;
        const hasDiffs =
          payload.diffs.some(diffCanBeTracked) && Boolean(workCycleId);
    ...
    resumeSession: async (sessionId) => {
      ...
      try {
        ...
        const latestSession = get().sessionsById[sessionId] ?? currentSession;
        ...
        if (supportsNativeResume) {
          ...
          if (
            ...
            resumedSession.modes.some(
              (mode) => mode.id === latestSession.modeId && !mode.disabled,
            )
          ) {
          ...
          resumeContextPending = getSessionTranscriptLength(latestSession) > 0;
        }
          ...
          replaceSessionTranscript(
            {
              ...
              resumeContextPending,
              runtimeState: "live",
              persistedCreatedAt: latestSession.persistedCreatedAt ?? null,
              persistedUpdatedAt: latestSession.persistedUpdatedAt ?? null,
              persistedTitle: latestSession.persistedTitle ?? null,
              customTitle: latestSession.customTitle ?? null,
              persistedPreview: latestSession.persistedPreview ?? null,
              persistedMessageCount:
    ...
    loadSession: async (sessionId) => {
      ...
      if (existing) {
        set((state) => ({
          activeSessionId: sessionId,
          selectedRuntimeId:
            state.sessionsById[sessionId]?.runtimeId ?? state.selectedRuntimeId,
          sessionOrder: touchSessionOrder(state.sessionOrder, sessionId),
        }));
      ...
      try {
        ...
        get().applySessionError({
          ...
          message: getAiErrorMessage(error, "Failed to load the session."),
        });
      }
    },
    ...
    setModel: async (modelId, sessionId) => {
      await applyAgentConfigMutation({
        ...
        applyLocal: (session) => applyLocalModelSelection(session, modelId),
        applyRemote: async (session) => {
          const modelConfig = getModelConfigOption(session);
          return modelConfig &&
            modelConfig.options.some((option) => option.value === modelId)
            ? aiSetConfigOption(session.sessionId, modelConfig.id, modelId)
    ...
    setMode: async (modeId, sessionId) => {
      await applyAgentConfigMutation({
        ...
        applyLocal: (session) => applyLocalModeSelection(session, modeId),
    ...
    setComposerParts: (parts, sessionId) => {
      ...
      set((state) => {
        ...
        const mentionIds = new Set(
          parts
            .filter(
              (p): p is Extract<AIComposerPart, { type: "mention" }> =>
                p.type === "mention",
            )
            .map((p) => p.noteId),
        );
        const folderPaths = new Set(
          parts
            .filter(
              (p): p is Extract<AIComposerPart, { type: "folder_mention" }> =>
        ...
        const fileMentionPaths = new Set(
          parts
            .filter(
              (p): p is Extract<AIComposerPart, { type: "file_mention" }> =>
                p.type === "file_mention",
            )
            .map((p) => p.path),
        );
    ...
    enqueueMessage: (sessionId, item) =>
      set((state) => ({
        queuedMessagesBySessionId: {
          ...state.queuedMessagesBySessionId,
          [sessionId]: [
            ...(state.queuedMessagesBySessionId[sessionId] ?? []).filter(
              (queuedItem) => queuedItem.id !== item.id,
            ),
            item,
          ],
        ...
        return {
          ...
          queuedMessageEditBySessionId: {
            ...
            [sessionId]: {
              ...
              previousAttachments: session.attachments.map(cloneAttachment),
            },
          },
        };
      }),
    ...
    cancelQueuedMessageEdit: (sessionId) => {
      ...
      set((state) => {
        ...
        return {
          ...
          composerPartsBySessionId: finalizedEdit.nextComposerPartsBySessionId,
          queuedMessagesBySessionId:
      ...
      if (queuedMessageEdit) {
        ...
        set((state) => {
          const finalizedEdit = finalizeQueuedMessageEditState(
            state,
            resolvedSessionId,
            shouldRequeueEditedMessage ? updatedQueuedItem : undefined,
          );
          if (!finalizedEdit) {
            return state;
          }
    ...
    sendQueuedMessageNow: async (sessionId, messageId) => {
      ...
      if (!currentSession || !queuedItem || queuedItem.status === "sending") {
        return;
      }
    ...
    stopStreaming: async (sessionId) => {
      ...
      stopPromise = (async () => {
        ...
        const shouldPauseQueue =
          activeQueuedMessage != null ||
          (get().queuedMessagesBySessionId[resolvedSessionId]?.length ?? 0) > 0;

        if (shouldPauseQueue) {
          if (activeQueuedMessage) {
            restoreActiveQueuedMessage(resolvedSessionId, (item) => ({
              ...item,
              status: "queued",
            }));
          }
        ...
        if (_pendingStopBySessionId.get(resolvedSessionId) === stopPromise) {
      ...
      try {
        ...
        set((state) => {
          ...
          return {
            sessionsById: {
              ...state.sessionsById,
              [sessionId]: appendSessionError(currentSession, message),
            },
            sessionOrder: touchSessionOrder(state.sessionOrder, sessionId),
          };
        });
      ...
      try {
        ...
        set((state) => {
          ...
          return {
            sessionsById: {
              ...state.sessionsById,
              [resolvedSessionId]: appendSessionError(currentSession, message),
            },
            sessionOrder: touchSessionOrder(
              state.sessionOrder,
              resolvedSessionId,
    ...
    rejectEditedFile: async (sessionId, identityKey) => {
      try {
        ...
        set((state) => {
          ...
          const freshTracked =
            findTrackedFileInAccumulatedSession(currentSession, identityKey) ??
            tracked;
          ...
          const sessionAfterRemove =
            restoreAction.kind === "skip"
              ...
              : (() => {
                  const { undoData } = actionLogRejectAll(freshTracked);
    ...
    rejectAllEditedFiles: async (sessionId) => {
      ...
      set((state) => {
        ...
        for (const [key, file] of Object.entries(current)) {
          if (!undoIdentityKeys.has(key) && !removedIdentityKeys.has(key)) {
            remaining[key] = file;
          }
        }
    ...
    keepEditedFile: (sessionId, identityKey) => {
      set((state) => {
        ...
        const updated = removeTrackedFileFromActionLog(session, identityKey);
      ...
      if (updatedSession) {
        void persistSession(updatedSession);
        closeReviewIfSessionHasNoPendingEdits(sessionId, updatedSession);
      }
    },
      ...
      if (decision === "accepted") {
        ...
        if (vaultPath) {
          const change = await aiRestoreTextFile({
            ...
            path: tracked.path,
            previousPath:
              tracked.originPath !== tracked.path ? tracked.originPath : null,
        ...
        if (
          ...
          if (hunkUndoSnapshot) {
            cleaned = setActionLogUndo(cleaned, {
              buffers: [],
              snapshots: {
                [hunkUndoSnapshot.identityKey]: hunkUndoSnapshot.snapshot,
              },
              timestamp: Date.now(),
            });
          }
    ...
    undoLastReject: async (sessionId) => {
      ...
      for (const [identityKey, snapshot] of Object.entries(snapshots)) {
        try {
          const currentHash = await aiGetTextFileHash(vaultPath, snapshot.path);
          if (!canRestoreRejectUndoSnapshot(snapshot, currentHash)) {
            continue;
          }

          await restoreRejectUndoSnapshotAndReload(vaultPath, snapshot);
      ...
      if (Object.keys(restoredSnapshots).length > 0) {
        ...
        set((state) => {
          ...
          const existingFiles = getAccumulatedTrackedFiles(currentSession);
          const restoredFiles = {
            ...existingFiles,
            ...restoredSnapshots,
          };
          const restoredKeys = new Set(Object.keys(restoredSnapshots));
          const restoredPaths = new Set(
            Object.values(restoredSnapshots).map((snapshot) => snapshot.path),
          );
            ...
            Object.keys(remainingSnapshots).length === 0
              ...
              : {
                  buffers: lastRejectUndo.buffers.filter(
                    (buffer) => !restoredPaths.has(buffer.path),
                  ),
    ...
    newSession: async (runtimeId) => {
      ...
      const nextRuntimeId =
        runtimeId ?? get().selectedRuntimeId ?? getDefaultRuntimeId(runtimes);
      if (!nextRuntimeId) return;
      ...
      try {
        ...
        if (
          !runtimeSupportsCapability(runtimes, nextRuntimeId, "create_session")
        ) {
          set((state) => ({
            runtimeConnectionByRuntimeId: setRuntimeConnectionState(
              state.runtimeConnectionByRuntimeId,
    ...
    deleteSession: async (sessionId) => {
      ...
      const historySessionId = targetSession?.historySessionId ?? sessionId;
      clearStaleStreamingCheck(sessionId);
      ...
      const nextActiveId =
        state.activeSessionId === sessionId
          ...
          : state.activeSessionId;
      const nextSelectedRuntimeId =
        (nextActiveId ? nextSessionsById[nextActiveId]?.runtimeId : null) ??
        state.selectedRuntimeId;
      set({
        sessionsById: nextSessionsById,
        ...
        activeQueuedMessageBySessionId: nextActiveQueuedMessageBySessionId,
        pausedQueueBySessionId: nextPausedQueueBySessionId,
        interruptedTurnStateBySessionId: nextInterruptedTurnStateBySessionId,
      });
    ...
    attachCurrentNote: (note) => {
      ...
      set((state) => ({
        sessionsById: updateSessionById(state, activeSessionId, (session) => ({
          ...session,
          attachments: withUniqueAttachment(
            session.attachments,
    ...
    attachSelectionFromEditor: () => {
      ...
      const nextParts = appendSelectionMentionPart(currentParts, {
        ...
        label: buildSelectionLabel(currentSelection.text, startLine, endLine),
        path: selectionPath,
    ...
    forkSession: async (sessionId) => {
      ...
      try {
        ...
        const runtime =
          state.runtimes.find((r) => r.runtime.id === session.runtimeId) ??
          state.runtimes[0];
        if (!runtime) return;
        ...
        const forkedSession: AIChatSession = {
          ...
          isPersistedSession: true,
          runtimeState: "persisted_only",
          resumeContextPending: (session.persistedMessageCount ?? 0) > 0,
          messages: [],
          attachments: [],
...
export function initializeChatStoreRuntime() {
  ...
  stopChatStorageSync = subscribeSafeStorage((event) => {
    ...
    if (
      event.key === getAutoContextStorageKey(useVaultStore.getState().vaultPath)
    ) {
      ...
      autoContextSyncTimer = window.setTimeout(() => {
        ...
        useChatStore
          .getState()
          .syncAutoContextForVault(useVaultStore.getState().vaultPath);
      }, 80);
    }
  });
