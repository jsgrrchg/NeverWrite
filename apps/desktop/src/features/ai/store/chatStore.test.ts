...
function createMockTrackedFilePatch(
  ...
  return {
    linePatch,
    textRangePatch: buildTextRangePatchFromTexts(oldText, newText, linePatch),
  };
}
...
function mockRustTrackedFilePatches(
  ...
  invokeMock.mockImplementation(async (command, args) => {
    if (command === "compute_tracked_file_patches") {
      return await resolver(getMockTrackedFilePatchInputs(args), callIndex++);
    }
...
describe("chatStore", () => {
  ...
  it("persists auto context per vault path", () => {
    ...
    expect(localStorage.getItem(getAutoContextKey("/vaults/one"))).toBe("true");
  ...
  it("keeps the previous visible work cycle while its permission buffer is unresolved", async () => {
    ...
    useChatStore.getState().setComposerParts(createTextParts("Second turn"));
    await useChatStore.getState().sendMessage();
  ...
  it("keeps accumulated tracked edits visible when a new prompt starts before the previous review is resolved", async () => {
    ...
    let updatedSession = useChatStore.getState().sessionsById[activeSessionId]!;
    expect(updatedSession.visibleWorkCycleId).toBe("cycle-old");
    expect(updatedSession.activeWorkCycleId).not.toBe("cycle-old");
    ...
    expect(
      oldCycleTracked == null || Object.keys(oldCycleTracked).length === 0,
    ).toBe(true);
  });
  ...
  it("reloads an open markdown note when agent diffs are consolidated", async () => {
    ...
    expect(editorState._pendingForceReloads.has("notes/current")).toBe(true);
  ...
  it("refreshes the active live session catalog on initialize when ACP lists it empty", async () => {
    ...
    invokeMock.mockImplementation(async (command, args) => {
      ...
      if (command === "ai_load_session") {
        expect((args as { sessionId?: string } | undefined)?.sessionId).toBe(
  ...
  it("loads only the latest persisted transcript page for the active live session on initialize", async () => {
    ...
    expect(invokeMock).toHaveBeenCalledWith("ai_load_session_history_page", {
      vaultPath: "/vault",
      sessionId: "codex-session-existing",
      startIndex: 20,
      limit: 60,
  ...
  it("does not reorder session history when flushing streamed deltas", async () => {
    ...
    const secondSession = cloneSessionForTest(activeSession, "codex-session-2");
  ...
  it("migrates virtualized row UI state when a detached session is resumed", async () => {
    ...
    expect(
      useChatStore.getState().sessionsById[resumedSessionId]?.messageOrder,
    ).toEqual([messageId]);
    expect(
      useChatStore.getState().sessionsById[resumedSessionId]?.messagesById?.[
        messageId
      ]?.content,
    ).toBe("Resume work");
    expect(
    ...
    expect(
      serializeComposerParts(
        useChatStore.getState().composerPartsBySessionId[resumedSessionId] ??
          [],
      ),
    ).toBe("Resume draft");
    expect(
    ...
    expect(
      useChatStore.getState().queuedMessageEditBySessionId[resumedSessionId],
    ).toMatchObject({
      item: editedQueuedMessage,
      previousComposerParts: previousDraftParts,
    });
    expect(
      useChatStore.getState().activeQueuedMessageBySessionId[resumedSessionId],
    ).toMatchObject({
      item: queuedMessage,
      nextItemId: editedQueuedMessage.id,
    });
    ...
    expect(
      useChatStore.getState().queuedMessagesBySessionId[detachedSessionId],
    ).toBeUndefined();
    expect(
      useChatStore.getState().queuedMessageEditBySessionId[detachedSessionId],
    ).toBeUndefined();
    expect(
      useChatStore.getState().activeQueuedMessageBySessionId[detachedSessionId],
    ).toBeUndefined();
    expect(
  ...
  it("drains the next queued message when the session returns to idle", async () => {
    ...
    expect(
      useChatStore.getState().activeQueuedMessageBySessionId[activeSessionId]
        ?.item,
    ).toMatchObject({
  ...
  it("pauses the queue on cancel and resumes it only after the next manual send", async () => {
    ...
    expect(
      useChatStore
        .getState()
        .queuedMessagesBySessionId[activeSessionId]?.map((item) => item.id),
    ).toEqual(["queued-1"]);
    expect(
  ...
  it("waits for an in-flight stop before sending the next manual message", async () => {
    ...
    const stopPromise = useChatStore.getState().stopStreaming(activeSessionId);
    await Promise.resolve();

    useChatStore
      .getState()
      .setComposerParts(createTextParts("Manual redirect"));
    const sendPromise = useChatStore.getState().sendMessage(activeSessionId);
  ...
  it("dispatches the first manual send after stop without requiring a second submit", async () => {
    ...
    expect(
      useChatStore.getState().interruptedTurnStateBySessionId[activeSessionId]
  ...
  it("ignores late assistant activity after stop until a fresh turn starts", async () => {
    ...
    expect(useChatStore.getState().sessionsById[activeSessionId]?.status).toBe(
      "idle",
    );
    expect(
      useChatStore
  ...
  it("retries a failed queued message without duplicating the user turn", async () => {
    ...
    expect(
      session.messages.filter(
        (message) =>
          message.role === "user" && message.content === "Retry me once",
      ),
    ).toHaveLength(1);
    expect(
      useChatStore.getState().queuedMessagesBySessionId[activeSessionId],
  ...
  it("interrupts the current turn and sends the queued message immediately when sending it now", async () => {
    ...
    expect(
      invokeMock.mock.calls.filter(([command]) => command === "ai_cancel_turn"),
    ).toHaveLength(1);
    expect(
      invokeMock.mock.calls.filter(
        ([command, payload]) =>
  ...
  it("removes a queued message from the visible list once it has been sent", async () => {
    ...
    useChatStore.setState((state) => ({
      queuedMessagesBySessionId: {
        ...state.queuedMessagesBySessionId,
        [activeSessionId]: [createQueuedMessage("queued-1", "Only queued")],
      },
    }));
  ...
  it("requeues an edited message only once when stale queue state already contains the same item", async () => {
    ...
    useChatStore.setState((state) => ({
      ...
      sessionsById: {
        ...
        [activeSessionId]: {
          ...state.sessionsById[activeSessionId]!,
          status: "waiting_permission",
          attachments: staleEditedItem.attachments.map((attachment) => ({
            ...attachment,
          })),
        },
      },
    ...
    expect(
      useChatStore.getState().queuedMessagesBySessionId[activeSessionId]?.[1]
        ?.content,
    ).toBe("Second updated");
  });
  ...
  it("keeps the edited files buffer after message completion transitions the session to idle", async () => {
    ...
    const workCycleId =
      useChatStore.getState().sessionsById[activeSessionId]!.activeWorkCycleId!;
  ...
  it("replaces a resolved visible work cycle when a new turn starts", async () => {
    ...
    const oldCycleTracked2 =
      session.actionLog?.trackedFilesByWorkCycleId?.["cycle-old"];
    expect(
      oldCycleTracked2 == null || Object.keys(oldCycleTracked2).length === 0,
    ).toBe(true);
  });
  ...
  it("starts a new work cycle without waiting for deprecated tracked-file precomputation", async () => {
    ...
    const session = useChatStore.getState().sessionsById[activeSessionId]!;
    expect(
      invokeMock.mock.calls.some(([command]) => command === "ai_send_message"),
    ).toBe(true);
    expect(
      invokeMock.mock.calls.some(
    ...
    const oldCycleTracked =
      firstWorkCycleId == null
        ? null
        : session.actionLog?.trackedFilesByWorkCycleId?.[firstWorkCycleId];
    expect(
      oldCycleTracked == null || Object.keys(oldCycleTracked).length === 0,
    ).toBe(true);
  });
  ...
  it("keeps earlier hunks after a user edit and a later Rust-refined agent edit", async () => {
    ...
    const reviewItems = deriveReviewItems(buffer, new Set(["/notes/file.md"]));
  ...
  it("does not let a late Rust refinement collapse earlier hunks on an accumulated file", async () => {
    ...
    firstRefinement.resolve([createMockTrackedFilePatch(baseText, midText)]);
    await drainRustTrackedFileWork();
  ...
  it("auto-removes a carried entry when a later cycle reverts the file", async () => {
    ...
    // Start cycle B
    useChatStore.getState().setComposerParts(createTextParts("Revert turn"));
    await useChatStore.getState().sendMessage();
  ...
  it("applies user text edits from the editor while preserving untouched agent spans", async () => {
    ...
    expect(agentSpan.currentTo).toBeLessThanOrEqual(line1Start + "BBB".length);
  });
  ...
  it("propagates user text edits to every session tracking the same file", async () => {
    ...
    const firstSession = useChatStore.getState().sessionsById[firstSessionId]!;
  ...
  it("keepEditedFile removes only the targeted entry and keeps other work-cycle entries intact", async () => {
    ...
    useChatStore.setState((state) => {
      ...
      let actionLog = emptyActionLogState();
      actionLog = setTrackedFilesForWorkCycle(actionLog, visibleWorkCycleId, {
        [visibleEntry.identityKey]: visibleEntry,
      });
      actionLog = setTrackedFilesForWorkCycle(actionLog, hiddenWorkCycleId, {
  ...
  it("keeps the review tab open after rejectEditedFile resolves the last pending file", async () => {
    ...
    expect(
      useEditorStore
        .getState()
        .tabs.find(
          (tab) => isReviewTab(tab) && tab.sessionId === activeSessionId,
        ),
    ).toBeDefined();
  });
  ...
  it("consolidates successful rejects before a later rejectAll failure", async () => {
    ...
    expect(
      session.actionLog?.lastRejectUndo?.snapshots?.["/vault/src/parser.rs"],
    ).toBeUndefined();
  });
  ...
  it("keeps only failed snapshots in undoLastReject after a partial restore", async () => {
    ...
    expect(
      session.actionLog?.lastRejectUndo?.snapshots?.["/vault/src/watcher.rs"],
    ).toBeUndefined();
  });
  ...
  it("resumes the active persisted history into a live ACP session", async () => {
    ...
    invokeMock.mockImplementation(async (command) => {
      ...
      if (command === "ai_create_session") return emptyCatalogSessionPayload;
  ...
  it("aborts resume when the required persisted transcript page cannot be loaded", async () => {
    ...
    expect(
      ...
      kind: "error",
      content: "Failed to load the full saved transcript before resuming.",
    });
  });
  ...
  it("rehydrates Claude histories with their runtime and resumes them natively", async () => {
    ...
    const claudeRuntimePayload = [
      ...runtimePayload,
      {
        runtime: {
          ...
          name: "Claude ACP",
          description: "Claude runtime embedded as an ACP sidecar.",
          capabilities: ["create_session", "list_sessions", "resume_session"],
        },
        models: [],
        modes: [],
        config_options: [],
    ...
    expect(state.sessionsById["persisted:history-claude-1"]).toBeUndefined();
  ...
  it("recreates an empty Claude session with additionalRoots on the first external file send", async () => {
    ...
    useChatStore.getState().setComposerParts(createTextParts("Review this"));
  ...
  it("does not persist the edited files buffer as part of session history", async () => {
    ...
    expect(historyPayload?.history).not.toHaveProperty("visibleWorkCycleId");
  });
  ...
  it("coalesces repeated history persistence requests in the same microtask", async () => {
    ...
    expect(payload?.history?.messages?.at(-1)?.content).toBe("Second error");
  });
  ...
  it("marks a persisted session as resuming while reconnecting it to ACP", async () => {
    ...
    let resolveCreateSession: ((value: typeof sessionPayload) => void) | null =
      null;
  ...
  it("does not persist empty sessions and ignores empty persisted histories", async () => {
    ...
    expect(useChatStore.getState().sessionOrder).toEqual(["codex-session-1"]);
    expect(
  ...
  it("applies agent changes while the session is busy", async () => {
    ...
    await useChatStore.getState().setConfigOption("reasoning_effort", "high");

    expect(
      invokeMock.mock.calls.filter(([command]) => command === "ai_set_model"),
    ).toHaveLength(0);
    expect(
      invokeMock.mock.calls.filter(([command]) => command === "ai_set_mode"),
    ).toHaveLength(1);
    expect(
      invokeMock.mock.calls.filter(
        ...
        ([command, payload]) =>
          ...
          (() => {
            ...
            const input = payload.input as
              ...
              | undefined;
            return (
              input?.option_id === "model" && input?.value === "wide-model"
            );
          })(),
      ),
    ).toBe(true);
  ...
  it('keeps setModel observably aligned with setConfigOption("model") for live ACP sessions', async () => {
    ...
    invokeMock.mockImplementation(async (command, args) => {
      if (command === "ai_set_config_option") {
        ...
        if (input?.option_id === "model") {
          return {
            ...sessionPayload,
            session_id: input.session_id ?? sessionPayload.session_id,
    ...
    const modelSession = useChatStore.getState().sessionsById[firstSessionId]!;
    const configSession =
    ...
    const modelSelectionCalls = invokeMock.mock.calls.filter(
      ([command, args]) =>
        ...
        args !== null &&
        "input" in args &&
        (args.input as { option_id?: string; value?: string }).option_id ===
          "model" &&
  ...
  it("preserves a fresher session-updated model change when ai_set_config_option returns stale data", async () => {
    ...
    expect(
      finalSession.configOptions.find((option) => option.id === "model")?.value,
    ).toBe("wide-model");
    expect(
  ...
  it("accepts updates for the active workspace session even when its stored vault path is stale", async () => {
    ...
    useChatStore.getState().upsertSession({
      ...
      modelId: "wide-model",
      configOptions: existing.configOptions.map((option) =>
        option.id === "model" ? { ...option, value: "wide-model" } : option,
      ),
    });
  ...
  it("keeps model and mode config option values aligned with incoming session updates", async () => {
    ...
    expect(
      merged.configOptions.find((option) => option.category === "model")?.value,
    ).toBe("wide-model");
  });
  ...
  it("refreshes the agent catalog when loading an existing live session with empty options", async () => {
    ...
    const session = useChatStore.getState().sessionsById[emptyLiveSessionId]!;
    expect(useChatStore.getState().activeSessionId).toBe(emptyLiveSessionId);
    expect(session.models).toHaveLength(1);
    expect(session.modes).toHaveLength(1);
    expect(session.configOptions).not.toHaveLength(0);
  });
  ...
  it("supports targeting a non-active session for local draft and attachment mutations", async () => {
    ...
    useChatStore
      .getState()
      .upsertSession(cloneSessionForTest(activeSession, secondSessionId), true);

    useChatStore
      .getState()
      .setComposerParts(createTextParts("second explicit"), secondSessionId);
    useChatStore.getState().attachNote(
      {
        id: "notes/two",
        title: "Note Two",
  ...
  it("resumes a restored session before applying agent settings", async () => {
    ...
    const resumeSession = vi.fn(async (sessionId: string) => {
      const persistedSession = useChatStore.getState().sessionsById[sessionId]!;
      useChatStore.setState((state) => {
        const resumedSession = cloneSessionForTest(
          activeSession,
          resumedSessionId,
          {
            historySessionId: persistedSession.historySessionId ?? sessionId,
            runtimeState: "live",
            isPersistedSession: false,
    ...
    invokeMock.mockImplementation(async (command, args) => {
      if (command === "ai_set_mode") {
        expect(
          (args as { sessionId?: string; modeId?: string } | undefined)
            ?.sessionId,
        ).toBe(resumedSessionId);
        expect(
          (args as { sessionId?: string; modeId?: string } | undefined)?.modeId,
        ).toBe("review-mode");
        return {
          ...sessionPayload,
          session_id: resumedSessionId,
    ...
    expect(
      restored?.configOptions.find((option) => option.id === "model")?.value,
    ).toBe("wide-model");
    expect(
      restored?.configOptions.find((option) => option.id === "reasoning_effort")
        ?.value,
    ).toBe("high");
    expect(
      invokeMock.mock.calls.filter(
  ...
  it("does not simulate a persisted model change when resumeSession cannot make it live", async () => {
    ...
    await useChatStore.getState().setModel("wide-model", persistedSessionId);
  ...
  it("supports targeting a non-active live session for send, user input and stop actions", async () => {
    ...
    useChatStore
      .getState()
      .setComposerParts(createTextParts("Send to second"), secondSessionId);
    ...
    expect(
      state.sessionsById[secondSessionId]?.messages.some(
        (message) =>
          message.role === "user" && message.content === "Send to second",
      ),
    ).toBe(true);
  ...
  it("attachSelectionFromEditor inserts a selection_mention composer part", async () => {
    ...
    const parts =
      useChatStore.getState().composerPartsBySessionId[activeSessionId] ?? [];
    const selectionParts = parts.filter((p) => p.type === "selection_mention");
  ...
  it("attachSelectionFromEditor shows single line label", async () => {
    ...
    expect(
      selectionPart?.type === "selection_mention" ? selectionPart.label : null,
    ).toBe("(7)  single line");
  });
  ...
  it("attachSelectionFromEditor deduplicates identical selections", async () => {
    ...
    expect(parts.filter((p) => p.type === "selection_mention")).toHaveLength(1);
  });
  ...
  it("keeps move-only tracked files pending after accepting the last text hunk", async () => {
    ...
    const session = createSessionWithTrackedFiles("session-move-only", [file]);
    const projection = buildReviewProjection(file);
  ...
  it("resolveReviewHunks resolves the expanded overlap closure returned by the canonical index", async () => {
    ...
    const session = createSessionWithTrackedFiles("session-overlap", [file]);
    const projection = buildReviewProjection(file);
  ...
  it("normalizes transcript metadata when upserting a session", () => {
    ...
    const session = useChatStore.getState().sessionsById["normalized-session"]!;
  ...
  it("keeps normalized transcript metadata in sync for hot runtime handlers", async () => {
    ...
    expect(session.messageIndexById?.["assistant-hot"]).toBe(
      session.messages.findIndex((message) => message.id === "assistant-hot"),
    );
