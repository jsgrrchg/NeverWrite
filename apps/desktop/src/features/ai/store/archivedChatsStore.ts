...
export const getArchivedChatsStorageKey = (vaultPath: string) =>
    `neverwrite.chats.archived:${vaultPath}`;

export function readArchivedChats(
    vaultPath: string | null,
): ArchivedChatEntries {
    if (!vaultPath) return {};
    try {
        const value = JSON.parse(
            safeStorageGetItem(getArchivedChatsStorageKey(vaultPath)) ?? "null",
        );
        if (
            value?.version !== 1 ||
            !value.entries ||
...
export function getArchiveRoot(
    ...
    while (root.parentSessionId && !seen.has(root.sessionId)) {
        ...
        const parent = Object.values(sessions).find((candidate) =>
            getAiSessionLookupKeys(candidate).includes(root.parentSessionId!),
        );
...
export function getArchiveIdentity(session: AIChatSession) {
    return (
        session.historySessionId || session.sessionId.replace(/^persisted:/, "")
    );
}
...
export function isSessionArchived(
    ...
    return Boolean(
        entries[getArchiveIdentity(root)] || entries[root.sessionId],
    );
}
...
export const useArchivedChatsStore = create<ArchivedChatsStore>((set, get) => {
    const update = (entries: ArchivedChatEntries) => {
        ...
        if (
            !safeStorageSetItem(
                getArchivedChatsStorageKey(path),
    ...
    return {
        ...
        setVaultPath: (vaultPath) => {
            if (get().vaultPath !== vaultPath)
                set({ vaultPath, entries: readArchivedChats(vaultPath) });
        },
        archive: (id) => {
            if (id && !get().entries[id])
                update({ ...get().entries, [id]: { archivedAt: Date.now() } });
        },
        unarchive: (id) => {
        ...
        reconcile: (ids, complete) => {
            ...
            update(
                Object.fromEntries(
                    Object.entries(get().entries).filter(([id]) =>
                        keep.has(id),
                    ),
                ),
            );
