...
export function useEditableFileResource({
  ...
  const hasExternalConflict = useEditorStore((state) => {
    ...
    return relativePath ? state.fileExternalConflicts.has(relativePath) : false;
  });
  ...
  useEffect(() => {
    ...
    syncLiveFilePathState(selectEditorWorkspaceTabs(useEditorStore.getState()));
  ...
  const saveFile = useCallback(
    async (targetTab: EditableFileTab, content: string) => {
      ...
      pendingLocalOpIdByPathRef.current.set(targetTab.relativePath, localOpId);
      ...
      try {
        ...
        pendingLocalOpIdByPathRef.current.delete(targetTab.relativePath);
  ...
  const handleLocalContentChange = useCallback(
    (content: string) => {
      ...
      const lastSaved =
        lastSavedContentByPathRef.current.get(currentTab.relativePath) ??
        currentTab.content;
      const store = useEditorStore.getState();
  ...
  useEffect(() => {
    const unsubscribe = useEditorStore.subscribe((state, prev) => {
      ...
      const reloadVersion = state._fileReloadVersions?.[relativePath] ?? 0;
      ...
      const trackedDiffBase = trackedMatch?.trackedFile.isText
        ...
        : null;
      const normalizedCurrentContent = normalizeReloadText(currentContent);
      const normalizedIncomingContent = normalizeReloadText(incomingContent);
      const safeTrackedReviewReload =
        incomingOrigin === "external" &&
        trackedCurrentText != null &&
        normalizedIncomingContent === normalizeReloadText(trackedCurrentText) &&
      ...
      if (safeTrackedReviewReload) {
        logDebug(
          ...
          "accepting external reload as tracked review sync",
          {
            ...
            trackedCurrentLength: trackedCurrentText?.length ?? null,
          },
          {
        ...
        lastSavedContentByPathRef.current.set(relativePath, incomingContent);
      ...
      if (hasLocalUnsavedChanges && !isForced) {
        ...
        useEditorStore.getState().markFileExternalConflict(relativePath);
        return;
      }
