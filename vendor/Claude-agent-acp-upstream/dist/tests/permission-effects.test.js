...
describe("Claude permission response effects", () => {
    const build = (toolName, durableChangeSet, input = {}, displayName, allowPersistentOptions = true, availableModes = []) => buildClaudePermissionOptions({
        toolName,
        displayName,
        input,
        cwd: "/workspace",
    ...
    it.each([
        ...
        const result = permissionResult({ outcome: { outcome: "selected", optionId } }, "Skill", input, "tool-skill", build("Skill", undefined, input));
    ...
    it("applies a generated WebFetch effect and an exact MCP provider effect", () => {
        ...
        }, "WebFetch", webInput, "tool-web", build("WebFetch", undefined, webInput));
        ...
        }, "mcp__demo__deploy", { target: "staging" }, "tool-mcp", build("mcp__demo__deploy", mcpChangeSet), mcpChangeSet);
        expect(fallback.behavior === "allow" && fallback.updatedPermissions).toEqual(mcpChangeSet?.updates);
    });
    it.each([
    ...
    ])("applies the exact provider filesystem effect for %s", (toolName, input, ruleTool) => {
        ...
        }, toolName, input, `tool-${toolName}`, build(toolName, changeSet, input), changeSet);
    ...
    it("suppresses generated durable effects when an ask rule forced the prompt", () => {
        expect(build("WebFetch", undefined, { url: "https://example.com" }, undefined, false)).toHaveLength(2);
    ...
    it("allows EnterPlanMode temporarily without changing mode before the tool runs", () => {
        expect(permissionResult({ outcome: { outcome: "selected", optionId: PERMISSION_OPTION_ID.allowOnce } }, "EnterPlanMode", {}, "tool-enter-plan", build("EnterPlanMode"))).toEqual({
            behavior: "allow",
            updatedInput: {},
    ...
    it("applies the selected ExitPlanMode mode and maps rejection", () => {
        ...
        }, "ExitPlanMode", input, "tool-plan", offered);
        ...
        }, "ExitPlanMode", input, "tool-plan", acceptEditsOffered);
    ...
    it("interrupts the old query when ExitPlanMode clears context", () => {
        ...
        expect(permissionResult({
            outcome: {
                outcome: "selected",
                optionId: PERMISSION_OPTION_ID.exitPlanClearAuto,
            },
        }, "ExitPlanMode", input, "tool-plan", offered)).toEqual({
            behavior: "deny",
    ...
    it("maps one-time allow without remembered updates", () => {
        expect(permissionResult({ outcome: { outcome: "selected", optionId: PERMISSION_OPTION_ID.allowOnce } }, "Bash", { command: "pwd" }, "tool-1", build("Bash"))).toEqual({
            behavior: "allow",
    ...
    it("maps durable allow to the snapshotted provider effect", () => {
        ...
        const result = permissionResult({ outcome: { outcome: "selected", optionId: PERMISSION_OPTION_ID.allowWithUpdates } }, "Bash", { command: "npm test" }, "tool-2", build("Bash", changeSet), changeSet);
    ...
    it("distinguishes explicit reject from cancellation", () => {
        expect(permissionResult({ outcome: { outcome: "selected", optionId: PERMISSION_OPTION_ID.reject } }, "Bash", {}, "tool-3", build("Bash"))).toEqual({
            behavior: "deny",
            message: "User refused permission to run tool",
            toolUseID: "tool-3",
            decisionClassification: "user_reject",
        });
        expect(() => permissionResult({ outcome: { outcome: "cancelled" } }, "Bash", {}, "tool-3", build("Bash"))).toThrow("Tool use aborted");
    });
    it("fails closed for an unavailable or unknown selection", () => {
        expect(() => permissionResult({ outcome: { outcome: "selected", optionId: PERMISSION_OPTION_ID.allowWithUpdates } }, "Bash", {}, "tool-4", build("Bash"))).toThrow("Permission option was not offered");
        expect(() => permissionResult({ outcome: { outcome: "selected", optionId: "future" } }, "Bash", {}, "tool-4", build("Bash"))).toThrow("Permission option was not offered");
    });
});
