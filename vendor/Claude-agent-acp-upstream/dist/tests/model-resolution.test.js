...
describe("resolveModelPreference - resolvedModel matching", () => {
    it("matches a preference that equals an alias's resolvedModel exactly", () => {
        ...
        expect(resolveModelPreference(LIVE_SHAPED_MODELS, "claude-opus-4-8[1m]")?.value).toBe("opus[1m]");
        expect(resolveModelPreference(LIVE_SHAPED_MODELS, "claude-haiku-4-5-20251001")?.value).toBe("haiku");
    });
    ...
    it("every model's resolvedModel (when present) resolves back to that same row", async () => {
        ...
        try {
            ...
            for (const model of models) {
                ...
                if (!model.resolvedModel || model.value === "default")
                    continue;
