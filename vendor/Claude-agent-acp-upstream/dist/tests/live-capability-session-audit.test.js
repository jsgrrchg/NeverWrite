...
function toolStatusSequence(updates, toolCallId) {
    return updates
        .map(updateBody)
...
function summarizeRaw(messages) {
    const counts = {};
...
async function createHarness() {
    ...
    const realSettings = JSON.parse(await readFile(path.join(homedir(), ".claude", "settings.json"), "utf8"));
    ...
    if (!apiKey)
        throw new Error("apiKeyHelper returned an empty credential");
...
describe.skipIf(!enabled)("live Claude SDK to ACP capability audit", () => {
        ...
        try {
            const response = await harness.agent.prompt({
                ...
                prompt: [
                    {
                        type: "text",
                        text: "Use the Agent tool exactly once with run_in_background=true. Tell that agent to " +
            ...
            const announcementIndex = harness.events.findIndex((event) => event.kind === "update" && event.toolCallId === toolCallId);
            const permissionIndex = harness.events.findIndex((event) => event.kind === "permission" && event.toolCallId === toolCallId);
            ...
            expect(subagentRequest, "the background subagent must reach the ACP permission callback").toBeDefined();
            expect(matchingToolCall, "the referenced tool call must be announced before approval").toBeDefined();
    ...
    it("keeps ACP routing stable across a real /clear and follow-up", async () => {
        ...
        try {
            ...
            const resetIndex = resetMessages.findIndex((message) => message.type === "conversation_reset");
            const reset = resetIndex >= 0 ? resetMessages[resetIndex] : undefined;
            const providerIdsAfterReset = [
                ...new Set(resetMessages
                    .slice(resetIndex + 1)
                    .map(rawSessionId)
                    .filter(Boolean)),
            ];
        ...
        try {
            await harness.agent.prompt({
                ...
                prompt: [
                    {
                        type: "text",
                        text: "Use the Bash tool exactly once with command `sleep 2; printf LIVE_BACKGROUND_OK` " +
                            "and run_in_background=true. Do not use any other tool. After launch reply exactly LAUNCHED.",
                    },
                ],
            });
            await waitFor("a terminal task_updated or task_notification", () => harness.rawMessages.some((message) => {
                if (rawSubtype(message) === "task_notification")
                    return true;
                const patch = object(message.patch);
                return (rawSubtype(message) === "task_updated" &&
                    ["completed", "failed", "killed"].includes(String(patch?.status)));
            }), 15_000);
            const toolCall = harness.updates
                .map(updateBody)
                .find((update) => update.sessionUpdate === "tool_call" && object(update._meta)?.claudeCode !== undefined);
            const toolCallId = String(toolCall?.toolCallId ?? "");
            const rawTaskEvents = harness.rawMessages.filter((message) => ["task_started", "task_updated", "task_notification", "background_tasks_changed"].includes(rawSubtype(message) ?? ""));
            ...
            expect(rawTaskEvents.some((message) => ["task_updated", "task_notification"].includes(rawSubtype(message) ?? ""))).toBe(true);
        }
        finally {
            await harness.dispose();
        }
        ...
        try {
            const prompt = harness.agent.prompt({
                ...
                prompt: [
                    {
                        type: "text",
                        text: "Use the Bash tool exactly once to run `sleep 20`. Do not run it in the background " +
                            "and do not use any other tool.",
                    },
                ],
            });
            await waitFor("the Bash tool_call", () => harness.updates.some((update) => updateBody(update).sessionUpdate === "tool_call"), 20_000);
            await harness.agent.cancel({ sessionId: harness.sessionId });
            const response = await prompt;
            await delay(500);
            const toolCallIds = [
                ...new Set(harness.updates
                    .map(updateBody)
                    .filter((update) => update.sessionUpdate === "tool_call")
                    .map((update) => String(update.toolCallId))),
            ];
            const statuses = Object.fromEntries(toolCallIds.map((id) => [id, toolStatusSequence(harness.updates, id)]));
            printResult("cancel", {
                stopReason: response.stopReason,
                statuses,
                rawCounts: summarizeRaw(harness.rawMessages),
            ...
            for (const id of toolCallIds) {
                expect(statuses[id].some((status) => ["completed", "failed"].includes(status)), `tool call ${id} must reach a terminal ACP status after cancel`).toBe(true);
            }
        }
        finally {
            await harness.dispose();
