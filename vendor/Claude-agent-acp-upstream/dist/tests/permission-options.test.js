...
const permissionResult = (...args) => decodeClaudePermissionResponse(...args).permissionResult;
const rule = { toolName: "Bash", ruleContent: "npm test:*" };
describe("Claude permission options and response mapping", () => {
    const build = (toolName, durableChangeSet, input = {}, displayName, allowPersistentOptions = true, availableModes = [], contextUsedPercent) => buildClaudePermissionOptions({
        toolName,
        displayName,
        input,
        cwd: "/workspace",
    ...
    ])("ports the native static shell label for %s", (toolName, updates, expected) => {
    ...
    it("uses the shortest distinguishing parent for paths with the same basename", () => {
        ...
        expect(build("Bash", changeSet)[1]?.name).toBe("Yes, and allow codex-acp/src/ and claude-agent-acp/src/ access and ls -R commands");
    });
    it("does not repeat an identical path in the durable option", () => {
        ...
        expect(build("Bash", changeSet)[1]?.name).toBe("Yes, allow reading from src/ from this project");
    });
    it.each([
        "Bash",
        "PowerShell",
    ...
    it("uses generated native effects instead of applying an unrelated provider suggestion", () => {
        ...
        expect(build("WebFetch", bashChangeSet, { url: "https://example.com" })[1]?.name).toBe("Yes, and don't ask again for example.com");
    });
    it("preserves an MCP tool's exact provider suggestion", () => {
        ...
        const result = permissionResult({ outcome: { outcome: "selected", optionId: PERMISSION_OPTION_ID.allowWithUpdates } }, toolName, { value: "new" }, "tool-mcp-write", offered, changeSet);
        expect(result.behavior === "allow" && result.updatedPermissions).toEqual(changeSet?.updates);
    });
    it.each(["mcp__example__write_tool", "mcp__computer-use__click"])("offers no misleading durable option for unrepresentable %s suggestions", (toolName) => {
        const cases = [
            [
                {
                    type: "addRules",
    ...
    it.each(["mcp__example__write_tool", "mcp__computer-use__click"])("preserves the complete representable provider bundle for %s", (toolName) => {
        const changeSet = normalizeDurablePermissionChangeSet([
            {
                type: "addRules",
        ...
        expect(offered.map((option) => option.optionId)).toContain(PERMISSION_OPTION_ID.allowWithUpdates);
        const result = permissionResult({ outcome: { outcome: "selected", optionId: PERMISSION_OPTION_ID.allowWithUpdates } }, toolName, {}, `tool-${toolName}`, offered, changeSet);
        expect(result.behavior === "allow" && result.updatedPermissions).toEqual(changeSet?.updates);
    });
    it.each([
    ...
    ])("builds the native session label for %s", (toolName, input, durableName) => {
        const readOnly = toolName === "Read" || toolName === "Glob" || toolName === "Grep";
        const outside = Object.values(input).some((value) => typeof value === "string" && value.startsWith("/outside"));
        const changeSet = normalizeDurablePermissionChangeSet(readOnly
            ? [
                {
                    type: "addRules",
    ...
    it("resolves relative permission paths from the session cwd", () => {
        ...
        expect(build("Read", changeSet, { file_path: "/workspace/src/a.ts" })[1]?.name).toBe("Yes, during this session");
    });
    it("recognizes a backslash glob suffix in a provider path rule", () => {
        ...
        expect(build("Read", changeSet, { file_path: "/outside/a.ts" })[1]?.name).toBe("Yes, allow reading from outside/ during this session");
    });
    it("treats acceptEdits as broad only for ordinary write paths", () => {
        const changeSet = normalizeDurablePermissionChangeSet([
            { type: "setMode", mode: "acceptEdits", destination: "session" },
        ]);
        expect(build("Edit", changeSet, { file_path: "/workspace/a.ts" })[1]?.name).toBe("Yes, allow all edits during this session");
        expect(build("Edit", changeSet, { file_path: "/workspace/.claude/settings.json" })).toHaveLength(2);
    ...
    it("builds WebFetch and sandbox-host labels from provider data", () => {
        ...
        expect(build("WebFetch", webFetch, { url: "https://example.com/a" })[1]?.name).toBe("Yes, and don't ask again for example.com");
        ...
        expect(build("SandboxNetworkAccess", network, { host: "example.com" })[1]?.name).toBe("Yes, and don't ask again for example.com");
    });
    it("uses Claude Code's special session label for its own settings", () => {
        ...
        expect(build("Edit", changeSet, { file_path: ".claude/settings.json" })[1]?.name).toBe("Yes, and allow Claude to edit its own settings for this session");
    });
    ...
    ])("falls back through the native ExitPlanMode priority", (modes, optionId, name) => {
    ...
    ])("offers one clear-context ExitPlanMode choice using native priority", (modes, optionId, name) => {
        expect(build("ExitPlanMode", undefined, { plan: "Implement it" }, undefined, true, modes, 73)).toContainEqual(expect.objectContaining({ optionId, name }));
    });
    it("does not offer clear-context ExitPlanMode without a plan", () => {
        expect(build("ExitPlanMode", undefined, {}, undefined, true, ["auto"], 73).map((option) => option.optionId)).not.toContain(PERMISSION_OPTION_ID.exitPlanClearAuto);
    });
    it("recognizes real Computer Use MCP tools and preserves their exact durable suggestion", () => {
        ...
        }, toolName, { x: 10, y: 20 }, "tool-computer-use", offered, changeSet);
        expect(result.behavior === "allow" && result.updatedPermissions).toEqual(changeSet?.updates);
    });
    it("reuses the Computer Use MCP tool-call title", () => {
        expect(buildClaudePermissionPresentation({
    ...
    it("uses the provider display name for a matching generic-tool rule", () => {
        ...
        expect(build("mcp__demo__deploy", changeSet, {}, "Deploy")[1]?.name).toBe("Yes, and don't ask again for Deploy commands");
    });
