...
import { announceNativeSubagent, finishNativeSubagent, NativeSubagentRuntime, } from "../native-subagents.js";
function deferred() {
    let resolve;
...
function control(sessionUpdate, status, parentToolUseId) {
    return {
        sessionId: "root",
        update: {
            sessionUpdate,
...
describe("NativeSubagentRuntime lifecycle", () => {
    it("publishes a raced spawn exactly once", async () => {
        ...
        const published = [];
        const value = child();
        const publish = vi.fn(async (notification) => {
    ...
    it("publishes a raced terminal exactly once after the spawn", async () => {
        ...
        const publish = vi.fn(async (notification) => {
            published.push(notification);
            if (notification.update.sessionUpdate === "subagent_spawned")
    ...
    it("falls back to one ordinary failed tool call when no child ever starts", async () => {
        ...
        await expect(runtime.route(control("tool_call", "pending"), async () => { })).resolves.toBeNull();
        ...
        expect((fallback?.update._meta?.claudeCode).subagent).toBeUndefined();
    });
    ...
    it("discards unknown pending child updates during finishAll", async () => {
        const published = [];
        const delivered = [];
        const runtime = new NativeSubagentRuntime(true, "root", {}, async (notification) => {
            published.push(notification);
        }, { log: () => { } });
        const pending = {
            sessionId: "root",
        ...
        await expect(runtime.route(pending, async (value) => {
    ...
    it("creates a distinct ACP lifecycle when the SDK restarts the same task id", async () => {
        ...
        const launch = (toolCallId) => ({
            ...control("tool_call", "pending"),
            update: { ...control("tool_call", "pending").update, toolCallId },
        });
        const output = (parentToolUseId, text) => ({
            sessionId: "root",
            update: {
                sessionUpdate: "agent_message_chunk",
        ...
        await expect(runtime.route(output("launch-1", "first"), async () => { })).resolves.toMatchObject({ sessionId: "worker-1" });
        ...
        await expect(runtime.route(output("launch-2", "second"), async () => { })).resolves.toMatchObject({ sessionId: "worker-1:generation:2" });
        await runtime.finishTask("worker-1", "failed", async () => { }, "launch-1");
        expect(published.filter(({ update }) => update.sessionUpdate === "subagent_state_update")).toHaveLength(1);
        await expect(runtime.route(output("launch-2", "still running"), async () => { })).resolves.toMatchObject({ sessionId: "worker-1:generation:2" });
        ...
        const spawnedIds = published.flatMap(({ update }) => update.sessionUpdate === "subagent_spawned" ? [update.subagentSessionId] : []);
        const terminalIds = published.flatMap(({ update }) => update.sessionUpdate === "subagent_state_update" ? [update.subagentSessionId] : []);
