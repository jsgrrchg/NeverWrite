...
describe.skipIf(!enabled)("live Claude SDK to ACP capability audit", () => {
        ...
        try {
            await harness.agent.prompt({
                ...
                prompt: [
                    {
                        type: "text",
                        text: "Use ScheduleWakeup exactly once with delaySeconds=60, reason='bounded ACP live audit', " +
            ...
            try {
                await waitFor("an autonomous wake-up result", () => harness.rawMessages.slice(afterScheduling).some((message) => {
                    const origin = object(message.origin);
                    return origin?.kind === "auto-continuation" || origin?.kind === "task-notification";
                }) || transcriptText(harness.updates).includes("LIVE_WAKE_OK"), 75_000);
            }
            catch (error) {
            ...
            expect(wakeMessages.some((message) => {
                const kind = object(message.origin)?.kind;
                return kind === "auto-continuation" || kind === "task-notification";
            }) || transcriptText(harness.updates).includes("LIVE_WAKE_OK")).toBe(true);
        }
        finally {
    ...
    it("records whether a real refusal produces retraction or fallback messages", async () => {
        ...
        try {
            ...
            const refusalMessages = harness.rawMessages.filter((message) => ["model_refusal_fallback", "model_refusal_no_fallback"].includes(rawSubtype(message) ?? ""));
            const superseding = harness.rawMessages.filter((message) => Array.isArray(message.supersedes) && message.supersedes.length > 0);
            const notifications = harness.rawMessages.filter((message) => rawSubtype(message) === "notification");
            printResult("refusal", {
        ...
        try {
            ...
            await harness.agent.prompt({
                ...
                prompt: [
                    {
                        type: "text",
                        text: "Do all of these steps using Bash: " +
