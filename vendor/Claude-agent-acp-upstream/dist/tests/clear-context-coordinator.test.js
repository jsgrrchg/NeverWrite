...
function testHost(oldSession, freshSession) {
...
describe("continuePlanInFreshContext", () => {
    it("moves the pending ACP turn and its session preferences to a fresh query", async () => {
        ...
        const reset = { toolUseId: "tool-plan", plan: "Ship it", mode: "auto" };
        ...
        expect(host.restartSession).toHaveBeenCalledWith(expect.objectContaining({
            cwd: "/workspace",
        ...
        expect(host.continuationMessage).toHaveBeenCalledWith("public-session", "Ship it", turn.promptUuid);
        expect(freshSession.input.push).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it("does not resurrect original preferences after the session returns to defaults", async () => {
        ...
        await continuePlanInFreshContext("public-session", oldSession, { toolUseId: "tool-plan", plan: "Ship it", mode: "default" }, host);
    ...
    it("restores disabled Fast mode when a fresh session starts with it enabled", async () => {
        ...
        const host = testHost(oldSession, freshSession);
        await continuePlanInFreshContext("public-session", oldSession, { toolUseId: "tool-plan", plan: "Ship it", mode: "auto" }, host);
    ...
    it("does not publish or continue a restart cancelled while creating the fresh session", async () => {
        ...
        const host = testHost(oldSession, freshSession);
        let finishRestart;
        vi.mocked(host.restartSession).mockImplementation(() => new Promise((resolve) => {
            finishRestart = () => resolve(freshSession);
        }));
        const controller = new AbortController();
        const restart = continuePlanInFreshContext("public-session", oldSession, { toolUseId: "tool-plan", plan: "Ship it", mode: "auto" }, host, controller.signal);
    ...
    it("rejects a stale session before closing its query", async () => {
        ...
        await expect(continuePlanInFreshContext("public-session", oldSession, { toolUseId: "tool-plan", plan: "Ship it", mode: "auto" }, host)).rejects.toThrow("Cannot clear context without an active ACP turn");
...
describe("ExitPlanCoordinator", () => {
    it("owns cancellation cleanup for a replacement created after close", async () => {
        ...
        const settleCancelledTurn = vi.fn((_old, _owner, cancelledTurn) => {
            cancelledTurn.settled = true;
        });
        let finishRestart;
        vi.mocked(baseHost.restartSession).mockImplementation(() => new Promise((resolve) => {
            finishRestart = () => {
        ...
        const coordinator = new ExitPlanCoordinator(host);
    ...
    it("settles a restart failure without classifying it as a query transport loss", async () => {
        ...
        const settleFailedTurn = vi.fn((_owner, failedTurn) => {
...
describe("observeExitPlanToolResults", () => {
    it("restores a rejected ExitPlanMode marker from authoritative stream metadata", () => {
        ...
        }, [{ type: "tool_result", tool_use_id: "tool-plan", is_error: true }], state);
    ...
    it("correlates an accepted plan result with its pending context reset", () => {
        ...
        expect(observeExitPlanToolResults({ type: "user" }, [{ type: "tool_result", tool_use_id: "tool-plan", is_error: true }], state)).toBe("tool-plan");
    });
});
