...
import { AUTO_MODE_FALLBACK, SessionModeManager } from "../session-mode.js";
...
function createNotifications(updates) {
    return {
        sessionUpdate: vi.fn(async (update) => {
            updates.push(update);
        }),
        logError: vi.fn(),
    };
}
function createManager(session, updates = []) {
...
describe("session mode", () => {
    ...
    it("rejects invalid and unavailable modes before touching the query", async () => {
        ...
        await expect(manager.selectMode(SESSION_ID, "bypassPermissions")).rejects.toThrow("Mode bypassPermissions is not available in this session");
    ...
    it("falls Auto back to Accept edits and warns only once per session", async () => {
        ...
        expect(updates.filter((notification) => notification.update.sessionUpdate === "agent_message_chunk")).toHaveLength(1);
    });
    it("handles setSessionMode and publishes the updated config option", async () => {
        ...
        expect(notifications.sessionUpdate).not.toHaveBeenCalledWith(expect.objectContaining({
            update: expect.objectContaining({ sessionUpdate: "current_mode_update" }),
        }));
    });
    it("rejects missing and closed sessions", async () => {
        await expect(createManager(undefined).manager.setSessionMode({ sessionId: SESSION_ID, modeId: "plan" })).rejects.toThrow("Session not found");
        await expect(createManager(createSession({ queryClosed: true })).manager.setSessionMode({
    ...
    it("initializes the stable mode catalog and falls unsupported Auto back", async () => {
        ...
        expect(result.modes.availableModes.map((mode) => mode.id)).toEqual(expect.arrayContaining(["default", "acceptEdits", "plan", "auto"]));
    ...
    it("reconciles Auto after a model switch", async () => {
        ...
        const { manager } = createManager(session);
        await expect(manager.reconcileForModel(session, {
            value: "haiku",
            displayName: "Haiku",
            description: "",
        })).resolves.toBe(true);
