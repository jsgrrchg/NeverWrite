...
describe("session titles at turn-end", () => {
    ...
    function wrapTitleQuery(generator, title) {
        const generateSessionTitle = vi.fn(async (_d, _o) => title);
        return {
    ...
    async function* oneTurn(input, turns = 1) {
    ...
    it("generates a title at turn-end instead of publishing the raw prompt", async () => {
        ...
        const input = new Pushable();
        const { query, generateSessionTitle } = wrapTitleQuery(oneTurn(input), "Explain add() in hello.py");
    ...
    it("generates once per session, and a later turn can't fall back to the prompt", async () => {
        ...
        const input = new Pushable();
        const { query, generateSessionTitle } = wrapTitleQuery(oneTurn(input, 2), "Explain add() in hello.py");
        agent.sessions["test-session"] = mockSessionState({ query, input }, agent);
        await agent.prompt({
            sessionId: "test-session",
            prompt: [{ type: "text", text: LONG_PROMPT }],
    ...
    it("includes slash-command arguments in the title context", async () => {
        ...
        const input = new Pushable();
        const { query, generateSessionTitle } = wrapTitleQuery(oneTurn(input), "Investigate Kafka lag alert on prod-3");
        agent.sessions["test-session"] = mockSessionState({ query, input }, agent);
        await agent.prompt({
            sessionId: "test-session",
        ...
        expect(generateSessionTitle).toHaveBeenCalledWith(expect.stringContaining("/investigate-alert kafka lag on prod-3"), { persist: true });
