...
describe("providers", () => {
    ...
    it("rejects set for an unknown providerId", async () => {
        const [agent] = await createAgentMock();
        await expect(agent.unstable_setProvider({
            providerId: "openai",
            apiType: "anthropic",
    ...
    it("rejects set for an empty or non-http baseUrl", async () => {
        const [agent] = await createAgentMock();
        await expect(agent.unstable_setProvider({ providerId: "main", apiType: "anthropic", baseUrl: "" })).rejects.toMatchObject({ code: -32602 });
        await expect(agent.unstable_setProvider({
            providerId: "main",
            apiType: "anthropic",
    ...
    it("recreates loaded sessions with the new provider between turns", async () => {
        ...
        expect(mockQuery.mock.calls[1][0]).toEqual(expect.objectContaining({
            options: expect.objectContaining({
        ...
        expect(mockQuery.mock.calls[2][0]).toEqual(expect.objectContaining({
            options: expect.objectContaining({ resume: first.sessionId }),
        }));
        expect(mockQuery.mock.calls[2][0].options.env.ANTHROPIC_BASE_URL).toBe(process.env.ANTHROPIC_BASE_URL);
    });
    it("switches every loaded session through proxy replacements and back to native routing", async () => {
        ...
        for (let update = 0; update < 3; update += 1) {
            expect(resumedCalls.slice(update * 2, update * 2 + 2).map(([request]) => request.options.resume)).toEqual([first.sessionId, second.sessionId]);
        }
        expect(resumedCalls.slice(0, 2).map(([request]) => request.options.env.ANTHROPIC_BASE_URL)).toEqual(["https://first-gateway.example", "https://first-gateway.example"]);
        expect(resumedCalls.slice(2, 4).map(([request]) => request.options.env.ANTHROPIC_BASE_URL)).toEqual(["https://second-gateway.example", "https://second-gateway.example"]);
        expect(resumedCalls.slice(4).map(([request]) => request.options.env.ANTHROPIC_BASE_URL)).toEqual([process.env.ANTHROPIC_BASE_URL, process.env.ANTHROPIC_BASE_URL]);
    ...
    it("overrides user routing settings while preserving unrelated settings", async () => {
        ...
        expect(mockQuery.mock.calls[0][0].options.settings).toEqual(expect.objectContaining({
            apiKeyHelper: "",
            model: "claude-sonnet-4-6",
            env: expect.objectContaining({
                ANTHROPIC_BASE_URL: "https://gateway.example",
    ...
    it("waits for an active turn before recreating its session", async () => {
        ...
        agent.sessions[created.sessionId].turnQueue = [{ completion }];
    ...
    it("rejects vertex set without _meta project/region", async () => {
        ...
        await expect(agent.unstable_setProvider({
            ...
            apiType: "vertex",
            baseUrl: "https://vertex.example",
            _meta: { claudeCode: { vertex: { projectId: "my-project" } } },
        })).rejects.toMatchObject({ code: -32602 });
    });
