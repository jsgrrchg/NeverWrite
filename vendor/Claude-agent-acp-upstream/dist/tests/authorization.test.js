...
import { billsClaudeSubscription, CLAUDE_LOGIN_REQUIRED_MESSAGE, CLAUDE_SUBSCRIPTION_NOT_SUPPORTED_MESSAGE, CLAUDE_SUBSCRIPTION_NOT_SUPPORTED_REASON, } from "../hide-claude-auth.js";
...
describe("authorization", () => {
    beforeEach(() => {
        ...
        execFileSpy.mockImplementation((_file, args, cb) => {
    ...
    describe("--hide-claude-auth subscription guard", () => {
        ...
        function mockAccount(account, overrides = {}) {
            mockQuery.mockImplementation(() => makeMockQuery({
        ...
        function createRecordingAgent(capable, logger) {
            const updates = [];
            const extNotification = vi.fn(async () => { });
        ...
        describe("billsClaudeSubscription", () => {
            ...
            it.each(API_KEY_SOURCES)("classifies apiKeySource %s", (apiKeySource) => {
                const account = { subscriptionType: "pro", apiKeySource };
            ...
            it.each(API_PROVIDERS)("classifies apiProvider %s", (apiProvider) => {
                const account = { subscriptionType: "pro", apiProvider };
            ...
            it("bills the subscription when apiProvider and apiKeySource are absent", () => {
                const account = { subscriptionType: "max" };
            ...
            it("is false without a subscription", () => {
                expect(billsClaudeSubscription({})).toBe(false);
                expect(billsClaudeSubscription(undefined)).toBe(false);
                const withKey = { apiKeySource: "ANTHROPIC_API_KEY" };
        ...
        describe("fails closed at session creation", () => {
            ...
            async function assertVerdict(create, verdict) {
                if (verdict === "allowed") {
                    await expect(create()).resolves.toMatchObject({ sessionId: expect.any(String) });
                    return;
                }
                const error = await create().then(() => undefined, (thrown) => thrown);
            ...
            it.each(CASES)("refuses or allows $name on loadSession", async ({ account, verdict }) => {
                ...
                hideClaudeAuth();
                mockAccount(account);
                await assertVerdict(() => agent.loadSession({ sessionId: randomUUID(), ...newSessionParams() }), verdict);
            });
            it.each(CASES)("refuses or allows $name on resumeSession", async ({ account, verdict }) => {
                const [agent] = await createAgentMock();
                hideClaudeAuth();
                mockAccount(account);
                await assertVerdict(() => agent.resumeSession({ sessionId: randomUUID(), ...newSessionParams() }), verdict);
            });
        ...
        describe("on session creation", () => {
            ...
            it("reports the refused account before the refusal reaches the client", async () => {
                ...
                const order = [];
                extNotification.mockImplementation(async (method) => {
                    if (method === "_auth/status_update")
                        order.push("update");
                });
                await agent.newSession(newSessionParams()).then(() => order.push("resolved"), () => order.push("rejected"));
            ...
            it("rejects loadSession the same way", async () => {
                ...
                hideClaudeAuth();
                mockAccount({ subscriptionType: "pro" });
                await expect(agent.loadSession({ sessionId: randomUUID(), ...newSessionParams() })).rejects.toMatchObject(refusal);
            });
            it("rejects resumeSession the same way", async () => {
                const [agent] = await createAgentMock();
                hideClaudeAuth();
                mockAccount({ subscriptionType: "pro" });
                await expect(agent.resumeSession({ sessionId: randomUUID(), ...newSessionParams() })).rejects.toMatchObject(refusal);
            });
            ...
            it("warns once when the CLI reports no account at all", async () => {
                ...
                hideClaudeAuth();
                mockQuery.mockImplementation(() => makeMockQuery({ initializationResult: async () => ({ models }) }));
                await expect(agent.newSession(newSessionParams())).resolves.toMatchObject({
                    sessionId: expect.any(String),
                });
        ...
        describe("gateway authenticate validation", () => {
            it("accepts a call with no gateway payload and installs no override", async () => {
                ...
                await expect(agent.authenticate({ methodId: "gateway" })).resolves.toBeUndefined();
                expect(agent.resolveProviderConfig()).toBeNull();
                await expect(agent.authenticate({ methodId: "gateway", _meta: {} })).resolves.toBeUndefined();
                expect(agent.resolveProviderConfig()).toBeNull();
            });
            ...
            it("rejects a present but unusable gateway payload", async () => {
                const [agent] = await createAgentMock();
                await expect(agent.authenticate({ methodId: "gateway", _meta: { gateway: {} } })).rejects.toMatchObject({ code: INVALID_PARAMS_CODE });
                expect(agent.resolveProviderConfig()).toBeNull();
            });
        ...
        describe("on every prompt", () => {
            ...
            it("reports the account of a refused turn before the turn rejects", async () => {
                ...
                await agent.prompt(promptParams(sessionId)).then(() => order.push("resolved"), () => order.push("rejected"));
                expect(order).toEqual(["update", "rejected"]);
            ...
            it("publishes one failure row for two concurrent prompts", async () => {
                ...
                for (const outcome of outcomes) {
                    expect(outcome.reason).toMatchObject(refusal);
                }
        ...
        describe("respawns after a mid-session sign-out", () => {
            ...
            function scriptedQuery(args, account, script) {
                ...
                async function* messages() {
                    ...
                    for (;;) {
                        ...
                        for (const message of turns.shift() ?? [])
                            yield message;
                    }
                }
            ...
            async function signOutDuringTurn(agent) {
                const { sessionId } = await agent.newSession(newSessionParams());
            ...
            it("refuses the next turn when the CLI now holds a subscription", async () => {
                const [agent, updates] = createRecordingAgent(true);
                hideClaudeAuth();
                mockQueries({ account: SIGNED_IN_WITH_KEY, script: [[signOutResult()]] }, { account: { subscriptionType: "pro" }, script: [] });
                ...
                expect(failuresIn(updates).filter((failure) => failure.kind !== "advisory")).toHaveLength(1);
            });
            it("proceeds when the CLI now holds a key, on a clean failure state", async () => {
                const [agent] = createRecordingAgent(true);
                hideClaudeAuth();
                mockQueries({ account: SIGNED_IN_WITH_KEY, script: [[signOutResult()]] }, { account: SIGNED_IN_WITH_KEY, script: [[successResult()]] });
                const sessionId = await signOutDuringTurn(agent);
            ...
            it("refuses with the plain error and no second row when still signed out", async () => {
                const [agent, updates] = createRecordingAgent(true);
                hideClaudeAuth();
                mockQueries({ account: SIGNED_IN_WITH_KEY, script: [[signOutResult()]] }, { account: {}, script: [] });
                const sessionId = await signOutDuringTurn(agent);
                const rowsAfterSignOut = failuresIn(updates).length;
                const error = await agent.prompt(promptParams(sessionId)).then(() => undefined, (thrown) => thrown);
                expect(error.code).toBe(AUTH_REQUIRED_CODE);
                expect(error.message).toBe(`Authentication required: ${LOGIN_REQUIRED_MESSAGE}`);
                expect(error.data).toBeUndefined();
            ...
            it("reports the identity the respawned session logged back in with", async () => {
                ...
                mockQueries({ account: SIGNED_IN_WITH_KEY, script: [[signOutResult()]] }, { account: { apiKeySource: "apiKeyHelper" }, script: [[successResult()]] });
                const sessionId = await signOutDuringTurn(agent);
                await expect(agent.prompt(promptParams(sessionId))).resolves.toMatchObject({
                    stopReason: "end_turn",
                });
            ...
            function mockQuerySpawns(...spawns) {
                let spawn = 0;
                mockQuery.mockImplementation((args) => {
                    const current = spawns[Math.min(spawn, spawns.length - 1)];
                    spawn += 1;
                    const query = scriptedQuery(args, current.account ?? SIGNED_IN_WITH_KEY, current.script ?? []);
            ...
            it("starts a fresh query when the conversation was never persisted", async () => {
                const [agent] = createRecordingAgent(true);
                hideClaudeAuth();
                mockQuerySpawns({ account: SIGNED_IN_WITH_KEY, script: [[signOutResult()]] }, { initError: NO_CONVERSATION() }, { account: SIGNED_IN_WITH_KEY, script: [[successResult()]] });
                const sessionId = await signOutDuringTurn(agent);
                await expect(agent.prompt(promptParams(sessionId))).resolves.toMatchObject({
                    stopReason: "end_turn",
                });
            ...
            it("refuses the fresh query when the guard rejects the new account", async () => {
                const [agent] = createRecordingAgent(true);
                hideClaudeAuth();
                mockQuerySpawns({ account: SIGNED_IN_WITH_KEY, script: [[signOutResult()]] }, { initError: NO_CONVERSATION() }, { account: { subscriptionType: "pro" }, script: [] });
                const sessionId = await signOutDuringTurn(agent);
                await expect(agent.prompt(promptParams(sessionId))).rejects.toMatchObject(refusal);
            ...
            it("does not retry a resume that failed for another reason", async () => {
                const [agent] = createRecordingAgent(true);
                hideClaudeAuth();
                mockQuerySpawns({ account: SIGNED_IN_WITH_KEY, script: [[signOutResult()]] }, { initError: new Error("boom") });
                const sessionId = await signOutDuringTurn(agent);
            ...
            describe("recreates when the start-of-prompt probe finds another identity", () => {
                ...
                beforeEach(() => {
                    ...
                    execFileSpy.mockImplementation((...invocation) => {
                        const args = invocation[1];
                        const cb = invocation[invocation.length - 1];
                        if (args[1] === "status") {
                            if (probeStdout !== undefined)
                                cb(null, { stdout: probeStdout, stderr: "" });
                            return;
                        }
                        cb(null, { stdout: "", stderr: "" });
                    });
                ...
                function mockHeldFirstTurn(account, held, ...later) {
                    let spawn = 0;
                    mockQuery.mockImplementation((args) => {
                        const index = spawn++;
                        if (index > 0) {
                ...
                it("marks the session when the probe kind differs from the account", async () => {
                    ...
                    hideClaudeAuth();
                    probeStdout = PROBE_SUBSCRIPTION;
                    mockQueries({ account: SIGNED_IN_WITH_KEY, script: [[successResult()]] }, { account: { subscriptionType: "pro" }, script: [] });
                    const { sessionId } = await agent.newSession(newSessionParams());
                    await expect(agent.prompt(promptParams(sessionId))).resolves.toMatchObject({
                        stopReason: "end_turn",
                    });
        ...
        describe("reason-aware failure dedupe", () => {
            ...
            function controllerFor(agent, sessionId) {
                return new SessionFailureController({
                    ...
                    isCurrent: () => true,
                    sendUpdate: (notification) => agent.client.sessionUpdate(notification),
                    logger: { error: () => { } },
                });
            }
            const promptParams = (sessionId) => ({
        ...
        describe("on steer", () => {
            it("rejects a steer that would start a new turn", async () => {
                ...
                mockAccount(SIGNED_IN_WITH_KEY, { accountInfo: async () => ({ subscriptionType: "pro" }) });
                const { sessionId } = await agent.newSession(newSessionParams());
                await expect(agent.steer({ sessionId, prompt: [{ type: "text", text: "hello" }] })).rejects.toMatchObject(refusal);
                expect(agent.sessions[sessionId].turnQueue ?? []).toHaveLength(0);
            });
        });
...
describe("session failure reason field", () => {
    it("carries reason only when the publisher passes one", async () => {
        const { SessionFailureController, createSessionFailureState } = await import("../session-failure-extension.js");
        const capabilities = {
            _meta: { jetbrains: { air: { version: 1, capabilities: ["sessionFailure"] } } },
        };
        const withReason = [];
