...
import { AGENT_FILE_CHANGE_REPORT_MAX_BYTES, agentFileChangeReportRequestId, createFileChangeAuditSupport, createFileChangeAuditTurnState, FILE_CHANGE_AUDIT_TOOL_NAME, FILE_CHANGE_AUDIT_WIRE_TOOL_NAME, } from "../file-change-audit.js";
function auditToolHandler(support) {
    const server = support.mcpServer;
...
describe("agent file-change audit", () => {
    it("accepts only the versioned prompt request shape", () => {
        expect(agentFileChangeReportRequestId({
            jetbrains: {
                air: {
    ...
    it("continues Stop at most once and reports a missing tool call once", async () => {
        ...
        const publish = vi.fn(async (_result) => { });
        const support = createSupport({ state, publish });
        const hookOptions = { signal: new AbortController().signal };
        const preToolInput = (toolName) => ({
            hook_event_name: "PreToolUse",
            tool_name: toolName,
            tool_input: {},
            tool_use_id: "tool-1",
        });
        await expect(support.preToolUseHook(preToolInput(FILE_CHANGE_AUDIT_WIRE_TOOL_NAME), "tool-1", hookOptions)).resolves.toMatchObject({
        ...
        await expect(support.preToolUseHook(preToolInput("Bash"), "tool-1", hookOptions)).resolves.toMatchObject({
            hookSpecificOutput: { permissionDecision: "deny" },
        });
        await expect(support.preToolUseHook(preToolInput(FILE_CHANGE_AUDIT_WIRE_TOOL_NAME), "tool-1", hookOptions)).resolves.toMatchObject({
    ...
    it("publishes only the first unavailable terminal and rejects a late report", async () => {
        ...
        await expect(auditToolHandler(support).handler({ paths: ["src/late.ts"], complete: true })).resolves.toMatchObject({ isError: true });
        expect(publish).toHaveBeenCalledTimes(1);
    });
    it("normalizes roots, drops invalid paths, and deduplicates without truncation", async () => {
        ...
        const duplicateOnlyReports = [];
        const duplicateOnlyTool = auditToolHandler(createSupport({
    ...
    it("accepts canonical paths under a symlinked workspace root", async () => {
        ...
        try {
            ...
            state.phase = "collecting";
            const reports = [];
            const tool = auditToolHandler(createSupport({
                state,
                cwd: linkedRoot,
    ...
    it("caps path count and total UTF-8 bytes", async () => {
        ...
        const countReports = [];
        await auditToolHandler(createSupport({
            state: countState,
            cwd,
        ...
        if (countReports[0].status !== "reported")
            throw new Error("Expected a reported result");
        ...
        await auditToolHandler(createSupport({
            ...
            paths: Array.from({ length: 1000 }, (_, index) => `generated/${index}-${"x".repeat(280)}.txt`),
            complete: true,
        });
        ...
        if (byteReports[0].status !== "reported")
            throw new Error("Expected a reported result");
        expect(byteReports[0].paths.length).toBeLessThan(1000);
        expect(Buffer.byteLength(JSON.stringify(byteReports[0]), "utf8")).toBeLessThanOrEqual(AGENT_FILE_CHANGE_REPORT_MAX_BYTES);
    });
    it("fails open when report publication fails", async () => {
        ...
        await expect(unavailableSupport.finishUnavailable(unavailableState, "providerError")).resolves.toBeUndefined();
