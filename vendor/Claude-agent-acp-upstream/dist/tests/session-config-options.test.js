...
describe("session config options", () => {
    ...
    describe("setSessionConfigOption", () => {
        ...
        it("accepts max for effort_level and calls applyFlagSettings", async () => {
            const session = agent.sessions[SESSION_ID];
            session.modelInfos = [
                {
                    value: "claude-opus-4-5",
            ...
            expect(response.configOptions.find((o) => o.id === "effort_level")?.currentValue).toBe("max");
        });
        ...
        it("preserves max when the selected model supports it", async () => {
            const session = agent.sessions[SESSION_ID];
            session.effortLevel = "max";
            session.modelInfos = [
                {
                    value: "claude-opus-4-5",
                    displayName: "Claude Opus",
