...
function scriptedTurnQuery(args, account) {
...
describe("auth status mappers", () => {
    ...
    it("maps an API key source", () => {
        expect(fromAccountInfo({ apiProvider: "firstParty", apiKeySource: "ANTHROPIC_API_KEY" })).toEqual({
            kind: "api_key",
    ...
    it("reports an account with no identity signal as no information", () => {
        ...
        expect(fromAccountInfo({ apiProvider: "firstParty", tokenSource: "CLAUDE_CODE_OAUTH_TOKEN" })).toBeUndefined();
    });
    ...
    it("merges a poorer read into the same identity and replaces a different one", () => {
        ...
        expect(mergeAuthStatus(stored, { kind: "api_key", label: "Anthropic API key", detail: "env" })).toEqual({ kind: "api_key", label: "Anthropic API key", detail: "env" });
        // Key sources identify api_key payloads.
        expect(sameIdentity({ kind: "api_key", label: "Anthropic API key", detail: "apiKeyHelper" }, { kind: "api_key", label: "Anthropic API key", detail: "env" })).toBe(false);
    });
});
describe("auth status over ACP", () => {
    ...
    async function releaseStatus(index) {
    ...
    function statusCalls() {
        return execFileSpy.mock.calls.filter((call) => call[1][1] === "status");
    }
    function updates() {
    ...
    function lastPush() {
        return updates().at(-1)?.[1]?.authStatus;
    }
    ...
    async function scriptedSession(account) {
        mockQuery.mockImplementation((args) => scriptedTurnQuery(args, account));
        const { sessionId } = await agent.newSession({ cwd: process.cwd(), mcpServers: [] });
        return sessionId;
    }
    async function promptOnce(sessionId) {
    ...
    it("pushes again as soon as a field of the payload changes", async () => {
        ...
        await agent.newSession({ cwd: process.cwd(), mcpServers: [] });
        expect(updates().map((call) => call[1].authStatus.label)).toEqual(["Claude Max", "Claude Enterprise"]);
    });
    ...
    it("keeps the last known state when a probe is readable but empty", async () => {
        ...
        const published = { kind: "api_key", label: "Anthropic API key", detail: "env" };
    ...
    describe("when the CLI never answers", () => {
        ...
        const KNOWN = { kind: "api_key", label: "Anthropic API key", detail: "env" };
        let logger;
        beforeEach(() => {
            vi.useFakeTimers();
            ...
            extNotification = vi.fn().mockResolvedValue(undefined);
            agent = new ClaudeAcpAgent({ sessionUpdate: async () => { }, extNotification }, logger);
        ...
        it("bounds the probe at 5 s and keeps the last known state", async () => {
            ...
            expect(logger.error).toHaveBeenCalledWith("claude auth status did not answer within 5 s; the identity is not refreshed");
        });
        it("warns once however many probes time out", async () => {
            ...
            await vi.advanceTimersByTimeAsync(5000);
            expect(statusCalls()).toHaveLength(2);
            expect(logger.error.mock.calls.filter((call) => String(call[0]).includes("did not answer"))).toHaveLength(1);
        });
    });
});
