...
describe("Claude permission suggestion normalization", () => {
    ...
    it.each(["addRules", "replaceRules", "removeRules"])("supports and snapshots %s", (type) => {
        const suggestions = [
    ...
    it.each(["default", "acceptEdits", "bypassPermissions", "plan", "dontAsk", "auto"])("supports setMode %s", (mode) => {
        expect(normalizeDurablePermissionChangeSet([{ type: "setMode", mode, destination: "session" }])
            ?.updates).toEqual([{ type: "setMode", mode, destination: "session" }]);
    });
    it.each(["addDirectories", "removeDirectories"])("supports %s", (type) => {
        expect(normalizeDurablePermissionChangeSet([
            { type, directories: ["/one", "/two"], destination: "localSettings" },
        ])?.updates).toEqual([{ type, directories: ["/one", "/two"], destination: "localSettings" }]);
    });
    it.each([
    ...
    it("suppresses every durable option for a forced ask", () => {
        expect(normalizeDurablePermissionChangeSet([{ type: "addDirectories", directories: ["/work"], destination: "projectSettings" }], true)).toBeUndefined();
    });
});
describe("Claude permission ACP v1 presentation", () => {
    ...
    it.each(["Bash", "PowerShell"])("uses the %s tool name when no human command description is available", (toolName) => {
    ...
    it("keeps the WebSearch query out of the permission description", () => {
        ...
        expect(presentation.toolCall.title).toBe('Search "Agent Client Protocol ACP specification subagents v2"');
    });
    it.each([
        ...
        })._meta).toEqual({ permission: { version: 1, title } });
    });
    ...
    ])("does not use the %s operation subtitle as a permission explanation", (toolName, input, description) => {
        const presentation = buildClaudePermissionPresentation({
            toolName,
            input,
            toolUseID: `tool-${toolName}`,
