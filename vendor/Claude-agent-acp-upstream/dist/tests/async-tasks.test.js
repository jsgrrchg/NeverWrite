...
describe("AsyncTaskRuntime", () => {
        ...
        const task = backgroundBashTaskFromToolResult([
            {
                ...
                content: "Command running in background with ID: bpux8xmfg. Output is being written to: /private/tmp/claude/tasks/bpux8xmfg.output. You will be notified when it completes.",
            },
    ...
    it("correlates a background Bash result inside a batched structured message", () => {
        ...
        expect(backgroundBashTaskFromToolResult([
        ...
        ], toolUseResult, tools)).toMatchObject({
            taskId: "task-1",
            toolCallId: "bash",
            description: "npm run build",
        ...
        expect(backgroundBashTaskFromToolResult([
            {
                type: "tool_result",
                tool_use_id: "bash",
                content: "Command running in background with ID: task-1. Output is being written to: " +
                    "/private/tmp/claude/tasks/task-1.output. You will be notified when done.",
            },
        ], undefined, tools)).toMatchObject({
            taskId: "task-1",
            toolCallId: "bash",
            description: "npm run build",
            outputFilePath: "/private/tmp/claude/tasks/task-1.output",
    ...
    it("does not infer a background task without an unambiguous Bash result", () => {
        ...
        expect(backgroundBashTaskFromToolResult([{ type: "tool_result", tool_use_id: "read" }], toolUseResult, { read: { name: "Read", input: {} } })).toBeUndefined();
        expect(backgroundBashTaskFromToolResult([
            {
                type: "tool_result",
                tool_use_id: "bash",
                content: "Output is being written to: /private/tmp/claude/tasks/not-task-1.output. You will be notified",
            },
        ], toolUseResult, bash)?.outputFilePath).toBeUndefined();
        expect(backgroundBashTaskFromToolResult([{ type: "tool_result", tool_use_id: "bash" }], [{ backgroundTaskId: "task-1" }, { backgroundTaskId: "task-2" }], bash)).toBeUndefined();
    });
    it("uses a structured result tool id to disambiguate batched Bash results", () => {
        ...
        expect(backgroundBashTaskFromToolResult([{ type: "tool_result", tool_use_id: "bash-1", content: "first" }], { tool_use_id: "read", background_task_id: "task-2" }, {
            "bash-1": { name: "Bash", input: { command: "first" } },
            read: { name: "Read", input: {} },
        })).toBeUndefined();
    });
    it("detects the negotiated AIR capability", () => {
        expect(clientSupportsAsyncTasks({
    ...
    it("finishes remaining tasks and can retry a task whose terminal publication failed", async () => {
        ...
        const terminalIds = published.flatMap(({ update }) => update.sessionUpdate === "async_task_state_update" ? [update.asyncTaskId] : []);
