...
import { toolUpdateFromToolResult, createPostToolUseHook, createTaskHook, clearHookCallbacks, registerHookCallback, toolInfoFromToolUse, planEntries, applyTaskCreate, applyTaskList, applyTaskUpdate, parseTaskCreateOutput, parseTaskListOutput, taskStateToPlanEntries, } from "../tools.js";
describe("PostToolUse callback ownership", () => {
    it("clears only callbacks owned by the cancelled session", async () => {
        ...
        await hook({ hook_event_name: "PostToolUse", tool_input: {}, tool_response: {} }, "owned-first", context);
        await hook({ hook_event_name: "PostToolUse", tool_input: {}, tool_response: {} }, "owned-second", context);
...
describe("Bash terminal output", () => {
    ...
    describe("post-tool-use hook sends diff content for Edit tool", () => {
        it("should include content and locations from structuredPatch in hook update", async () => {
            ...
            ], "assistant", "test-session", toolUseCache, mockClientWithUpdate, mockLogger, { parentToolUseId: "parent-agent-tool" });
...
describe("applyTaskCreate / applyTaskUpdate", () => {
    ...
    it("updates fields by task ID and keeps insertion order in plan entries", () => {
        const state = new Map();
        applyTaskCreate(state, { subject: "A", description: "", activeForm: "Working on A" }, { task: { id: "1", subject: "A" } });
    ...
    it("rebuilds task state from TaskList while preserving richer local fields", () => {
        ...
        const output = parseTaskListOutput(JSON.stringify({
            tasks: [
...
describe("toAcpNotifications - Task* tools", () => {
    ...
    it("does not apply a logically failed TaskUpdate", () => {
        ...
        const taskState = new Map([["1", { subject: "Existing", status: "pending" }]]);
        const notifications = toAcpNotifications([
            {
                type: "tool_result",
                tool_use_id: "update-1",
...
describe("Agent/Task tool_result rendering from tool_use_result", () => {
    ...
    const PARTIAL_NOTE = "NOTE: this agent stopped at its 30-turn limit before finishing. The text below is PARTIAL output; treat it as incomplete. Send the agent a message (SendMessage) to let it continue from where it stopped.";
...
describe("tool_result_meta non-execution stamping", () => {
    ...
    it("removes Claude's outer fence from a rejected ExitPlanMode explanation only", () => {
        ...
        ], "user", "test-session", toolUseCache, mockClient, mockLogger, { toolResultMeta: [{ id: "toolu_plan", non_execution_kind: "user-rejected" }] });
        expect(notifications).toHaveLength(1);
        expect(notifications[0].update).toMatchObject({
            sessionUpdate: "tool_call_update",
    ...
    it("preserves fenced output from tools other than ExitPlanMode", () => {
        ...
        ], "user", "test-session", { toolu_bash: bashToolUse }, mockClient, mockLogger);
...
describe("Skill tool rendering", () => {
    ...
    describe("_meta.claudeCode.skill in tool_call notification", () => {
        it("includes skill name in _meta.claudeCode when Skill tool is invoked", () => {
            ...
            ], "assistant", "test-session", {}, {}, mockLogger);
            expect(notifications[0]?.update).toMatchObject({
        ...
        it("omits skill from _meta.claudeCode when skill name is missing", () => {
            const notifications = toAcpNotifications([{ type: "tool_use", id: "toolu_6", name: "Skill", input: {} }], "assistant", "test-session", {}, {}, mockLogger);
            const meta = notifications[0]?.update?._meta?.claudeCode;
            expect(meta).toBeDefined();
            expect(meta.skill).toBeUndefined();
        });
    });
    describe("_meta.claudeCode.skillPath", () => {
        const skillMeta = (skill, cwd) => toAcpNotifications([{ type: "tool_use", id: "toolu_skill_path", name: "Skill", input: { skill } }], "assistant", "test-session", {}, {}, mockLogger, cwd ? { cwd } : undefined)[0]?.update?._meta?.claudeCode;
        let root;
        beforeEach(() => {
