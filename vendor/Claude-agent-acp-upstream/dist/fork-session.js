...
import { forkSession as forkClaudeSession, getSessionMessages, importSessionToStore, } from "@anthropic-ai/claude-agent-sdk";
import { createHash } from "node:crypto";
import { assistantMessageText } from "./session-failure-extension.js";
import { SessionTiming } from "./session-timing.js";
function forkPoint(meta) {
...
async function loadFullSessionHistory(sessionId, cwd) {
...
function assistantGroups(entries, messageIdForGrouping) {
...
function fingerprintOccurrenceOnBranch(targetUuid, expectedFingerprint, entries, groups, messageIdForGrouping) {
    const entriesByUuid = new Map(entries.flatMap((entry) => (typeof entry.uuid === "string" ? [[entry.uuid, entry]] : [])));
    const seenGroups = new Set();
    let occurrence = 0;
    let cursor = targetUuid;
    while (cursor) {
        ...
        if (entry.type === "assistant" &&
            ...
            if (messageId && !seenGroups.has(messageId)) {
                ...
                if (group && fingerprint(group.text) === expectedFingerprint)
                    occurrence++;
            }
        }
...
function resolveFromFullHistory(entries, candidates, point, messageIdForGrouping) {
    ...
    if (exact)
        return exact;
    if (!point.messageFingerprint || !point.messageOccurrence)
        return undefined;
    const fingerprintMatches = [...groups.values()].filter((group) => group.uuid && fingerprint(group.text) === point.messageFingerprint);
    ...
    if (fingerprintMatches.length === 1)
        return fingerprintMatches[0]?.uuid;
    const occurrenceMatches = fingerprintMatches.filter((group) => group.uuid &&
        fingerprintOccurrenceOnBranch(group.uuid, point.messageFingerprint, entries, groups, messageIdForGrouping) === point.messageOccurrence);
...
export async function forkSession(params, dependencies) {
    ...
    const fullHistoryUuid = messageUuid
        ? undefined
        : resolveFromFullHistory(await loadFullSessionHistory(params.sessionId, params.cwd), candidateIds, point, dependencies.messageIdForGrouping);
    timing.phase("full-history", ` resolution=${liveUuid ? "live" : messageUuid ? "active" : fullHistoryUuid ? "full" : "missing"}`);
    if (!messageUuid && !fullHistoryUuid) {
        throw RequestError.invalidParams({ messageId: point.messageId }, `Fork point message ${point.messageId} was not found in session ${params.sessionId}`);
    }
