...
function appendLimit(lines, label, window) {
    if (!window || window.utilization === null)
        return;
    lines.push(`**${escapeMarkdown(label)}** — **${window.utilization}%**${formatReset(window.resets_at)}`, "", `\`${usageBar(window.utilization)}\``, "");
}
...
export function formatUsageResponse(usage) {
    ...
    lines.push("", "---", "", "### This session", "", "| Cost | API time | Active |", "|:--|:--|:--|", `| $${usage.session.total_cost_usd.toFixed(2)} | ${formatDuration(usage.session.total_api_duration_ms)} | ${formatDuration(usage.session.total_duration_ms)} |`, "", "| Breakdown | Tokens |", "|:--|--:|", `| Input | ${formatCount(totals.input)} |`, `| Output | ${formatCount(totals.output)} |`, `| Cache read | ${formatCount(totals.cacheRead)} |`, `| Cache write | ${formatCount(totals.cacheWrite)} |`);
    if (usage.behaviors) {
