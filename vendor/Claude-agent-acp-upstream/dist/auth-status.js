...
export function authStatusCapability() {
    return {};
}
const NOT_LOGGED_IN = {
    kind: "none",
...
export function notLoggedInAuthStatus() {
...
export function gatewayAuthStatus(baseUrl) {
    let host;
    if (baseUrl) {
        try {
 ...
 */
export function accountInfoHasIdentitySignal(account) {
    if (!account)
        return false;
    return Boolean(account.apiKeySource ||
...
export function fromAccountInfo(account) {
...
export function fromCliStatus(stdout) {
    let parsed;
    try {
...
function mapAuthFields(fields) {
    ...
    if (subscriptionType) {
        ...
        const account = { plan: subscriptionType };
        if (email)
            account.email = email;
        if (organization)
 ...
 */
export function mergeAuthStatus(previous, next) {
 ...
 */
export function sameAuthStatus(a, b) {
    if (!a) {
        return false;
    }
