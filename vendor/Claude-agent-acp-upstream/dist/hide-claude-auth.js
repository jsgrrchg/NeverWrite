...
export function shouldHideClaudeAuth() {
...
export const CLAUDE_SUBSCRIPTION_NOT_SUPPORTED_MESSAGE = "This integration does not support using claude.ai subscriptions.";
...
export const CLAUDE_LOGIN_REQUIRED_MESSAGE = "Sign in with an API key or a Console account to use this integration.";
/**
 ...
 */
const API_KEY_SOURCES_ABOVE_SUBSCRIPTION = new Set([
    "ANTHROPIC_API_KEY",
    "apiKeyHelper",
    "/login managed key",
]);
 ...
 */
export function billsClaudeSubscription(account) {
 ...
 */
export function holdsNonSubscriptionCredential(account) {
    if (!account) {
        return false;
    }
    if (account.apiProvider !== undefined && account.apiProvider !== "firstParty") {
...
export function claudeSubscriptionNotSupportedError() {
    return RequestError.authRequired({ reason: CLAUDE_SUBSCRIPTION_NOT_SUPPORTED_REASON }, CLAUDE_SUBSCRIPTION_NOT_SUPPORTED_MESSAGE);
}
...
export function claudeLoginRequiredError() {
...
export function warnClaudeSubscriptionGuardDegraded(options) {
    ...
    warn(options.logger, `Session ${options.sessionId}: the --hide-claude-auth subscription guard is degraded ` +
 ...
 */
export async function refuseClaudeSubscriptionTurn(options) {
...
async function evaluateGuard(options) {
    ...
    try {
        ...
        options.logger.error(`Session ${options.sessionId}: the guard's onAccount hook threw:`, error instanceof Error ? error.message : String(error));
    }
...
async function readGuardAccount(options) {
    const { query } = options;
    ...
    try {
        ...
        options.logger.error(`Session ${options.sessionId}: accountInfo failed; skipping the subscription check for this turn:`, error instanceof Error ? error.message : String(error));
        return undefined;
    }
}
...
async function publishRefusal(sessionFailures, failure) {
