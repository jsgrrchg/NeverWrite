...
export const DESKTOP_PACKAGE_JSON_PATH = path.join(REPO_ROOT, "apps/desktop/package.json");
export const DESKTOP_TAURI_CONF_PATH = path.join(
  REPO_ROOT,
  "apps/desktop/src-tauri/tauri.conf.json",
);
export const DESKTOP_CARGO_TOML_PATH = path.join(REPO_ROOT, "apps/desktop/src-tauri/Cargo.toml");
...
export function readCargoPackageVersion(cargoTomlText) {
  ...
  throw new Error(`Could not find [package] version in ${DESKTOP_CARGO_TOML_PATH}.`);
}

export function collectVersionIssues({ packageJson, tauriConf, cargoToml }, tagVersion) {
  const issues = [];
  const versions = [packageJson, tauriConf, cargoToml];

  for (const [sourceName, value] of Object.entries({ packageJson, tauriConf, cargoToml })) {
    if (!isStrictSemver(value)) {
      issues.push(`${sourceName} version "${value}" is not strict semver (X.Y.Z).`);
    }
  }
...
export function getChangelogEntry(markdown, version) {
  return parseChangelogEntries(markdown).find((entry) => entry.version === version) ?? null;
}
