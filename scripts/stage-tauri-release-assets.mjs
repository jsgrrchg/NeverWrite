...
function parseArgs(argv) {
  ...
  for (const key of ["bundleRoot", "target", "version", "tag", "repo", "outputDir", "metadataOut"]) {
    if (!args[key]) {
      throw new Error(`Missing required argument --${key.replace(/[A-Z]/g, (match) => `-${match.toLowerCase()}`)}.`);
    }
  }
...
function main() {
  ...
  console.log(`Staged release assets for ${args.target} in ${args.outputDir}`);
