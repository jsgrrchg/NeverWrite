...
The backend combines built-in descriptors for Codex, Claude, Grok, Kilo, and OpenCode with active custom ACP definitions persisted in app data. Codex has vendored release inputs under `vendor/`; Claude is prepared from an exact npm dependency under `apps/desktop/runtimes/claude/`. Grok, Kilo, and OpenCode are integrated as external runtimes that must be found on `PATH` or configured by override. Custom descriptors are generated by the Rust runtime catalog and are not added to the renderer's built-in authentication catalog.
...
- The packaged Claude npm runtime is documented in
  ...
  Treat either kind of runtime update as compatibility work rather than an
