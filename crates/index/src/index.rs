...
impl VaultIndex {
    ...
    pub fn build(notes: Vec<NoteDocument>) -> Self {
        ...
        for note in &notes {
            ...
            let name = note
                ...
                .unwrap_or("")
                .to_lowercase();
            index.names.entry(name).or_default().push(note_id.clone());
    ...
    fn add_note(&mut self, note: NoteDocument) {
        ...
        // Registrar en names (nombre del archivo sin extensión, lowercase)
        let name = note
            .path
            .0
            .file_stem()
            ...
            .unwrap_or("")
            .to_lowercase();
        self.names.entry(name).or_default().push(note_id.clone());
    ...
    pub(crate) fn resolve_link_target(
        ...
        // Buscar por nombre
        let name_lower = link_text.to_lowercase();
        let candidates = self.names.get(&name_lower)?;

        if candidates.len() == 1 {
            return Some(candidates[0].clone());
        }

        // Desambiguar por proximidad de path (shortest path distance)
        self.closest_by_path(candidates, from_path)
    }
